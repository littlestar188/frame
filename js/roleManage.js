...
//var role = {
	...
	*/
	optShow:function(id){
		...
		if(hash.indexOf("2")!= -1 ){
			...
			html += '<a href="javascript:void(0)" class="btn btn-danger btn-xs disabled" id="btn-del" data-id="'+id+'">删除</a>&nbsp;&nbsp;';
		}
		//改
		...
		if(hash.indexOf("1") == -1 ||hash.indexOf("2") == -1 ||hash.indexOf("3") == -1  ||hash.indexOf("4") == -1){
	...
	*/
	...
	// judgePer:function(){
	// 	//resources/json/listFirstMenu.json
	...
	delRole:function(roleId){
		...
		//$('#role_table').on("click","#btn-del",function(){
			...
			if(confirm("确定删除这些记录吗？")){
				...
				$.ajax({
					//url:'resources/json/deleteOneRole.json',
	...
	*/	
	oneMenu:function(roleId){
		var that = this;
		$.ajax({
			//url:"resources/json/getOneRole.json",
	...
	*/
	...
	firstMenu:function(elem,flag,nextMenu,oneRoleData){
		var that = this;
 		...
 		var data;
 		var idArr = []
 	 	//$.get("resources/json/listFirstMenu.json",{"cache": false},function(res){
