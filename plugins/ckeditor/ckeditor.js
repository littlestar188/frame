...
(function() {
	window.CKEDITOR && window.CKEDITOR.dom || (window.CKEDITOR || (window.CKEDITOR = function() {
		...
		f = {
			...
			basePath: function() {
				...
				if (!b) for (var c = document.getElementsByTagName("script"), f = 0; f < c.length; f++) {
					var d = c[f].src.match(a);
					if (d) {
						b = d[1];
						break
					}
			...
			domReady: function() {
				...
				return function(d) {
					...
					if (1 == c.length) if (document.addEventListener) document.addEventListener("DOMContentLoaded", a, !1),
	...
	CKEDITOR.event.prototype = function() {
		...
		b.prototype = {
			getListenerIndex: function(a) {
				for (var b = 0,
				f = this.listeners; b < f.length; b++) if (f[b].fn == a) return b;
				return - 1
			}
		};
		return {
			...
			on: function(b, e, f, l, d) {
				...
				if (0 > g.getListenerIndex(e)) {
					...
					h.fn = e;
					h.priority = d;
					for (var v = g.length - 1; 0 <= v; v--) if (g[v].priority <= d) return g.splice(v + 1, 0, h),
					{
						removeListener: k
					};
					g.unshift(h)
			...
			fire: function() {
				...
				return function(d, h, k) {
					...
					if (g) {
						var v = g.listeners;
						if (v.length) for (var v = v.slice(0), t, w = 0; w < v.length; w++) {
							if (g.errorProof) try {
								t = v[w].call(this, k, h, b, l)
							} catch(p) {} else t = v[w].call(this, k, h, b, l); ! 1 === t ? m = 1 : "undefined" != typeof t && (h = t);
							if (a || m) break
						}
					}
	...
	function() {
		...
		CKEDITOR.tools = {
			...
			transformPlainTextToHtml: function(a, g) {
				...
				b = b.replace(/\n/g, "\x3cbr\x3e");
				d || (b = b.replace(new RegExp("\x3cbr\x3e(?\x3d\x3c/" + h + "\x3e)"),
				function(a) {
			...
			getUniqueId: function() {
				for (var a = "e",
				g = 0; 8 > g; g++) a += Math.floor(65536 * (1 + Math.random())).toString(16).substring(1);
				return a
			},
			...
			indexOf: function(a, g) {
				if ("function" == typeof g) for (var d = 0,
				b = a.length; d < b; d++) {
					if (g(a[d])) return d
				} else {
			...
			parseCssText: function(a, g, d) {
				...
				function(a, d, k) {
					g && (d = d.toLowerCase(), "font-family" == d && (k = k.replace(/\s*,\s*/g, ",")), k = CKEDITOR.tools.trim(k));
					b[d] = k
				});
				return b
			},
			...
			enableHtml5Elements: function(a, g) {
				for (var d = "abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup main mark meter nav output progress section summary time video".split(" "), b = d.length, h; b--;) h = a.createElement(d[b]),
				g && a.appendChild(h)
			},
			checkIfAnyArrayItemMatches: function(a, g) {
				for (var d = 0,
				b = a.length; d < b; ++d) if (a[d].match(g)) return ! 0;
				return ! 1
			},
			...
			getCsrfToken: function() {
				...
				if (!a || 40 != a.length) {
					var a = [],
					g = "";
					if (window.crypto && window.crypto.getRandomValues) a = new Uint8Array(40),
					window.crypto.getRandomValues(a);
					else for (var d = 0; 40 > d; d++) a.push(Math.floor(256 * Math.random()));
			...
			style: {
				parse: {
					...
					_findColor: function(a) {
						...
						return g = g.concat(d.filter(a.split(/\s+/),
						function(a) {
	...
	function(a) {
		...
		a.setCustomData = function(a, c) {
			var e = this.getUniqueId(); (f[e] || (f[e] = {}))[a] = c;
			return this
		};
	...
	CKEDITOR.dom.node.prototype = new CKEDITOR.dom.domObject, CKEDITOR.NODE_ELEMENT = 1, CKEDITOR.NODE_DOCUMENT = 9, CKEDITOR.NODE_TEXT = 3, CKEDITOR.NODE_COMMENT = 8, CKEDITOR.NODE_DOCUMENT_FRAGMENT = 11, CKEDITOR.POSITION_IDENTICAL = 0, CKEDITOR.POSITION_DISCONNECTED = 1, CKEDITOR.POSITION_FOLLOWING = 2, CKEDITOR.POSITION_PRECEDING = 4, CKEDITOR.POSITION_IS_CONTAINED = 8, CKEDITOR.POSITION_CONTAINS = 16, CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
		...
		getIndex: function(a) {
			...
			do a && c != this.$ && c.nodeType == CKEDITOR.NODE_TEXT && (m || b(c)) || (e++, m = c.nodeType == CKEDITOR.NODE_TEXT);
		...
		getPrevious: function(a) {
			var f = this.$,
			b;
			do b = (f = f.previousSibling) && 10 != f.nodeType && new CKEDITOR.dom.node(f);
			while (b && a && !a(b));
			return b
		},
		getNext: function(a) {
			var f = this.$,
			b;
			do b = (f = f.nextSibling) && new CKEDITOR.dom.node(f);
			while (b && a && !a(b));
			return b
		},
	...
	CKEDITOR.dom.document.prototype = new CKEDITOR.dom.domObject, CKEDITOR.tools.extend(CKEDITOR.dom.document.prototype, {
		...
		getByAddress: function(a, f) {
			...
			c = 0; b && c < a.length; c++) {
				var e = a[c];
				if (f) for (var m = -1,
				l = 0; l < b.childNodes.length; l++) {
					var d = b.childNodes[l];
	...
	function() {
		...
		function c(a) {
			for (var b = 0,
			c = 0,
			g = l[a].length; c < g; c++) b += parseFloat(this.getComputedStyle(l[a][c]) || 0, 10) || 0;
			return b
		}
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			breakParent: function(a, b) {
				...
				if (CKEDITOR.env.ie && !CKEDITOR.env.edge) {
					for (c = new CKEDITOR.dom.element("div"); e = g.getFirst();) e.$.style.backgroundColor && (e.$.style.backgroundColor = e.$.style.backgroundColor),
					c.append(e);
					c.insertAfter(this);
					c.remove(!0)
			...
			isIdentical: function(a) {
				...
				if (b.$.isEqualNode) return b.$.style.cssText = CKEDITOR.tools.normalizeCssText(b.$.style.cssText),
			...
			hasAttribute: function() {
				function a(b) {
					...
					case "checked":
						return !! this.$.checked;
					case "value":
						return b = this.getAttribute("type"),
						"checkbox" == b || "radio" == b ? "on" != this.$.value: !!this.$.value
					}
			...
			moveChildren: function(a, b) {
				...
				if (c != a) {
					var g;
					if (b) for (; g = c.lastChild;) a.insertBefore(c.removeChild(g), a.firstChild);
			...
			mergeSiblings: function() {
				...
				return function(b) {
					if (!1 === b || CKEDITOR.dtd.$removeEmpty[this.getName()] || this.is("a")) a(this, this.getNext(), !0),
			...
			removeStyle: function(a) {
				...
				this.$.style.cssText || this.removeAttribute("style");
					...
					for (var b = [], e = 0; e < c.length; e++) if (g) for (var f = 0; f < g.length; f++) b.push([a, c[e], g[f]].join("-"));
			...
			unselectable: function() {
				...
				if (CKEDITOR.env.ie) {
					...
					for (var a, b = this.getElementsByTag("*"), c = 0, g = b.count(); c < g; c++) a = b.getItem(c),
			...
			getDocumentPosition: function(a) {
				...
				if (document.documentElement.getBoundingClientRect && (CKEDITOR.env.ie ? 8 !== CKEDITOR.env.version: 1)) {
					...
					l = g.$.documentElement,
					p = l.clientTop || e.$.clientTop || 0,
					q = l.clientLeft || e.$.clientLeft || 0,
					r = !0;
					CKEDITOR.env.ie && (r = g.getDocumentElement().contains(this), g = g.getBody().contains(this), r = f && g || !f && r);
					r && (CKEDITOR.env.webkit || CKEDITOR.env.ie && 12 <= CKEDITOR.env.version ? (b = e.$.scrollLeft || l.scrollLeft, c = e.$.scrollTop || l.scrollTop) : (c = f ? e.$: l, b = c.scrollLeft, c = c.scrollTop), b = m.left + b - q, c = m.top + c - p)
				} else for (p = this, q = null; p && "body" != p.getName() && "html" != p.getName();) {
					b += p.$.offsetLeft - p.$.scrollLeft;
					c += p.$.offsetTop - p.$.scrollTop;
					p.equals(this) || (b += p.$.clientLeft || 0, c += p.$.clientTop || 0);
					for (; q && !q.equals(p);) b -= q.$.scrollLeft,
					c -= q.$.scrollTop,
					q = q.getParent();
					q = p;
					p = (m = p.$.offsetParent) ? new CKEDITOR.dom.element(m) : null
				}
				a && (m = this.getWindow(), p = a.getWindow(), !m.equals(p) && m.$.frameElement && (a = (new CKEDITOR.dom.element(m.$.frameElement)).getDocumentPosition(a), b += a.x, c += a.y));
			...
			scrollIntoParent: function(a, b, c) {
				var g, e, f, m;
				function l(b, g) { / body | html / .test(a.getName()) ? a.getWindow().$.scrollBy(b, g) : (a.$.scrollLeft += b, a.$.scrollTop += g)
				}
				function p(a, b) {
					var g = {
						x: 0,
					...
					c = a.getWindow();
					c.equals(b) || (c = p(CKEDITOR.dom.element.get(c.$.frameElement), b), g.x += c.x, g.y += c.y);
					return g
				}
				function q(a, b) {
				...
				f = a.getDocument();
				var r = "BackCompat" == f.$.compatMode;
				a instanceof CKEDITOR.dom.window && (a = r ? f.getBody() : f.getDocumentElement());
				...
				y = a.$.clientWidth;
				f = e.x - q(this, "left") - z.x || 0;
				m = e.y - q(this, "top") - z.y || 0;
				g = e.x + g + q(this, "right") - (z.x + y) || 0;
				e = e.y + A + q(this, "bottom") - (z.y + u) || 0; (0 > m || 0 < e) && l(0, !0 === b ? m: !1 === b ? e: 0 > m ? m: e);
				c && (0 > f || 0 < g) && l(0 > f ? f: g, 0)
			},
			...
			copyAttributes: function(a, b) {
				...
				for (var g = 0; g < c.length; g++) {
					...
					m;
					if (! (f in b)) if ("checked" == f && (m = this.getAttribute(f))) a.setAttribute(f, m);
					else if (!CKEDITOR.env.ie || this.hasAttribute(f)) m = this.getAttribute(f),
					null === m && (m = e.nodeValue),
					a.setAttribute(f, m)
				}
			...
			forEach: function(a, b, c) {
				...
				if (!1 !== g) {
					c = this.getChildren();
					for (var e = 0; e < c.count(); e++) g = c.getItem(e),
	...
	function() {
		function a(a, b) {
			...
			if (!this._.start) {
				this._.start = 1;
				if (g.collapsed) return this.end(),
				null;
				g.optimize()
			}
			...
			if (!a && !this._.guardLTR) {
				...
				B = c.type == CKEDITOR.NODE_ELEMENT ? c.getChild(k) : c.getNext();
				this._.guardLTR = function(a, b) {
					return (!b || !l.equals(a)) && (!B || !a.equals(B)) && (a.type != CKEDITOR.NODE_ELEMENT || !b || !a.equals(g.root))
				}
			}
			if (a && !this._.guardRTL) {
				var D = d.type == CKEDITOR.NODE_ELEMENT ? d: d.getParent(),
				F = d.type == CKEDITOR.NODE_ELEMENT ? e ? d.getChild(e - 1) : null: d.getPrevious();
				this._.guardRTL = function(a, b) {
					return (!b || !D.equals(a)) && (!F || !a.equals(F)) && (a.type != CKEDITOR.NODE_ELEMENT || !b || !a.equals(g.root))
				}
			}
			var G = a ? this._.guardRTL: this._.guardLTR;
			f = h ?
			function(a, b) {
				return ! 1 === G(a, b) ? !1 : h(a, b)
			}: G;
			this.current ? c = this.current[m](!1, n, f) : (a ? c.type == CKEDITOR.NODE_ELEMENT && (c = 0 < k ? c.getChild(k - 1) : !1 === f(c, !0) ? null: c.getPreviousSourceNode(!0, n, f)) : (c = d, c.type == CKEDITOR.NODE_ELEMENT && ((c = c.getChild(e)) || (c = !1 === f(d, !0) ? null: d.getNextSourceNode(!0, n, f)))), c && !1 === f(c) && (c = null));
			for (; c && !this._.end;) {
				this.current = c;
		...
		CKEDITOR.dom.walker.empty = function(a) {
			return function(b) {
				for (var g = 0,
				c = b.getChildCount(); g < c; ++g) if (!k(b.getChild(g))) return !! a;
				return ! a
			}
		};
	...
	function() {
		...
		function f(a, b, c, d, e) {
			...
			B = a.endOffset,
			D, F, G, I, H, K;
			if (u && C.type == CKEDITOR.NODE_TEXT && y.equals(C)) y = a.document.createText(y.substring(x, B)),
			c.append(y);
			else {
				C.type == CKEDITOR.NODE_TEXT ? u ? K = !0 : C = C.split(B) : 0 < C.getChildCount() ? B >= C.getChildCount() ? (C = C.getChild(B - 1), F = !0) : C = C.getChild(B) : I = F = !0;
				y.type == CKEDITOR.NODE_TEXT ? u ? H = !0 : y.split(x) : 0 < y.getChildCount() ? 0 === x ? (y = y.getChild(x), D = !0) : y = y.getChild(x - 1) : G = D = !0;
				for (var J = y.getParents(), E = C.getParents(), S = f(), O = J.length - 1, T = E.length - 1, M = c, V, da, ca, ga = -1, N = S; N <= O; N++) {
					da = J[N];
					ca = da.getNext();
					for (N != O || da.equals(E[N]) && O < T ? b && (V = M.append(da.clone(0, e))) : D ? k(da, M, !1, G) : H && M.append(a.document.createText(da.substring(x))); ca;) {
						if (ca.equals(E[N])) {
							ga = N;
							break
						}
				...
				M = c;
				for (N = S; N <= T; N++) if (c = E[N], ca = c.getPrevious(), c.equals(J[N])) b && (M = M.getChild(0));
				else {
					N != T || c.equals(J[N]) && T < O ? b && (V = M.append(c.clone(0, e))) : F ? k(c, M, !1, I) : K && M.append(a.document.createText(c.substring(0, B)));
					if (N > ga) for (; ca;) ca = k(ca, M, !0);
					M = V
				}
				u || h()
			}
		...
		function e(a) {
			return function() {
				var b;
				return this[a ? "getPreviousNode": "getNextNode"](function(a) { ! b && k(a) && (b = a);
		...
		CKEDITOR.dom.range.prototype = {
			...
			createBookmark2: function() {
				function a(b) {
					...
					e = g;
					var k = d;
					e = e.type != CKEDITOR.NODE_ELEMENT || 0 === k || k == e.getChildCount() ? 0 : e.getChild(k - 1).type == CKEDITOR.NODE_TEXT && e.getChild(k).type == CKEDITOR.NODE_TEXT;
					e && (g = g.getChild(d - 1), d = g.getLength());
					if (g.type == CKEDITOR.NODE_ELEMENT && 0 < d) {
						a: {
							for (e = g; d--;) if (k = e.getChild(d).getIndex(!0), 0 <= k) {
								d = k;
								break a
							}
							d = -1
					...
					if (g.type == CKEDITOR.NODE_TEXT) {
						e = g;
						for (k = 0; (e = e.getPrevious()) && e.type == CKEDITOR.NODE_TEXT;) k += e.getText().replace(CKEDITOR.dom.selection.FILLING_CHAR_SEQUENCE, "").length;
						e = k;
						g.getText() ? d += e: (k = g.getPrevious(c), e ? (d = e, g = k ? k.getNext() : g.getParent().getFirst()) : (g = g.getParent(), d = k ? k.getIndex(!0) + 1 : 0))
					}
					b.container = g;
					b.offset = d
				}
				...
				return function(c) {
					...
					c && (a(e), b(e, this.root), d || (a(k), b(k, this.root)));
					return {
			...
			enlarge: function(a, b) {
				...
				case CKEDITOR.ENLARGE_ELEMENT:
					...
					B.type == CKEDITOR.NODE_TEXT ? (F && (B = !CKEDITOR.tools.trim(B.substring(0, F)).length && B, x = !!B), B && ((y = B.getPrevious()) || (u = B.getParent()))) : (F && (y = B.getChild(F - 1) || B.getLast()), y || (u = B));
					for (u = c(u); u || y;) {
						if (u && !y) { ! C && u.equals(f) && (C = !0);
							if (e ? u.isBlockBoundary() : !h.contains(u)) break;
							x && "inline" == u.getComputedStyle("display") || (x = !1, C ? m = u: this.setStartBefore(u));
							y = u.getPrevious()
						}
						for (; y;) if (B = !1, y.type == CKEDITOR.NODE_COMMENT) y = y.getPrevious();
						else {
							if (y.type == CKEDITOR.NODE_TEXT) D = y.getText(),
							d.test(D) && (y = null),
							B = /[\s\ufeff]$/.test(D);
							else if ((y.$.offsetWidth > (CKEDITOR.env.webkit ? 1 : 0) || b && y.is("br")) && !y.data("cke-bookmark")) if (x && CKEDITOR.dtd.$removeEmpty[y.getName()]) {
								D = y.getText();
								if (d.test(D)) y = null;
								else for (var F = y.$.getElementsByTagName("*"), G = 0, I; I = F[G++];) if (!CKEDITOR.dtd.$removeEmpty[I.nodeName.toLowerCase()]) {
									y = null;
									break
								}
								y && (B = !!D.length)
							} else y = null;
							B && (x ? C ? m = u: u && this.setStartBefore(u) : x = !0);
							if (y) {
								B = y.getPrevious();
								if (!u && !B) {
									u = y;
					...
					u = y = null;
					C = x = !1;
					B.type == CKEDITOR.NODE_TEXT ? CKEDITOR.tools.trim(B.substring(F)).length ? x = !0 : (x = !B.getLength(), F == B.getLength() ? (y = B.getNext()) || (u = B.getParent()) : k(B, F) && (u = B.getParent())) : (y = B.getChild(F)) || (u = B);
					for (; u || y;) {
						if (u && !y) { ! C && u.equals(f) && (C = !0);
							if (e ? u.isBlockBoundary() : !h.contains(u)) break;
							x && "inline" == u.getComputedStyle("display") || (x = !1, C ? l = u: u && this.setEndAfter(u));
							y = u.getNext()
						}
						for (; y;) {
							B = !1;
							...
							B = /^[\s\ufeff]/.test(D);
								...
								if ((0 < y.$.offsetWidth || b && y.is("br")) && !y.data("cke-bookmark")) if (x && CKEDITOR.dtd.$removeEmpty[y.getName()]) {
									D = y.getText();
									if (d.test(D)) y = null;
									else for (F = y.$.getElementsByTagName("*"), G = 0; I = F[G++];) if (!CKEDITOR.dtd.$removeEmpty[I.nodeName.toLowerCase()]) {
										y = null;
										break
									}
									y && (B = !!D.length)
					...
					m && l && (f = m.contains(l) ? l: m, this.setStartBefore(f), this.setEndAfter(f));
					break;
				case CKEDITOR.ENLARGE_BLOCK_CONTENTS:
				case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
					u = new CKEDITOR.dom.range(this.root);
					h = this.root;
					u.setStartAt(h, CKEDITOR.POSITION_AFTER_START);
					...
					u = this.clone();
					u.collapse();
					u.setEndAt(h, CKEDITOR.POSITION_BEFORE_END);
					u = new CKEDITOR.dom.walker(u);
					u.guard = a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS ? e: S;
					H = E = K = null;
					u = u.lastForward();
					H = H || h;
			...
			shrink: function(a, b, c) {
				if (!this.collapsed) {
					...
					m = 1,
					l = 1;
					e && e.type == CKEDITOR.NODE_TEXT && (f ? f >= e.getLength() ? d.setStartAfter(e) : (d.setStartBefore(e), m = 0) : d.setStartBefore(e));
					k && k.type == CKEDITOR.NODE_TEXT && (h ? h >= k.getLength() ? d.setEndAfter(k) : (d.setEndAfter(k), l = 0) : d.setEndBefore(k));
					...
					m && (e = d[a == CKEDITOR.SHRINK_ELEMENT ? "lastForward": "next"]()) && this.setStartAt(e, b ? CKEDITOR.POSITION_AFTER_START: CKEDITOR.POSITION_BEFORE_START);
			...
			removeEmptyBlocksAtEnd: function() {
				...
				c = CKEDITOR.dom.walker.bookmark(!1);
				return function(b) {
					for (var c = this.createBookmark(), d = this[b ? "endPath": "startPath"](), e = d.block || d.blockLimit, k; e && !e.equals(d.root) && !e.getFirst(a(e));) k = e.getParent(),
			...
			moveToElementEditablePosition: function(a, b) {
				...
				for (var c = 0; a;) {
					...
					f = c,
					h = void 0;
					e.type == CKEDITOR.NODE_ELEMENT && e.isEditable(!1) && (h = e[b ? "getLast": "getFirst"](k));
					f || h || (h = e[b ? "getPrevious": "getNext"](k));
					a = h
				}
				return !! c
			},
			moveToClosestEditablePosition: function(a, b) {
				var c, d = 0,
				e, k, f = [CKEDITOR.POSITION_AFTER_END, CKEDITOR.POSITION_BEFORE_START];
				a ? (c = new CKEDITOR.dom.range(this.root), c.moveToPosition(a, f[b ? 0 : 1])) : c = this.clone();
				if (a && !a.is(CKEDITOR.dtd.$block)) d = 1;
				else if (e = c[b ? "getNextEditableNode": "getPreviousEditableNode"]()) d = 1,
			...
			_find: function(a, b) {
				...
				h,
				m;
				if (c && c.find) for (f = c.find(a), k = 0; k < f.count(); k++) if (c = f.getItem(k), b || !c.isReadOnly()) h = c.getPosition(d.startNode) & CKEDITOR.POSITION_FOLLOWING || d.startNode.equals(c),
				m = c.getPosition(d.endNode) & CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IS_CONTAINED,
				h && m && e.push(c);
				return e
			}
		}
		...
		a.prototype = {
			getNextParagraph: function(a) {
				...
				if (this._.nestedEditable) {
					if (g = this._.nestedEditable.iterator.getNextParagraph(a)) return this.activeFilter = this._.nestedEditable.iterator.activeFilter,
					g;
				...
				if (!this._.started) {
					...
					f = p.startPath();
					var q = p.endPath(),
					r = !p.collapsed && c(p, f.block),
					z = !p.collapsed && c(p, q.block, 1);
					p.shrink(CKEDITOR.SHRINK_ELEMENT, !0);
					r && p.setStartAt(f.block, CKEDITOR.POSITION_BEFORE_END);
					z && p.setEndAt(q.block, CKEDITOR.POSITION_AFTER_START);
					f = p.endContainer.hasAscendant("pre", !0) || p.startContainer.hasAscendant("pre", !0);
				...
				for (this._.nextNode = null; q;) {
					var r = 0,
					z = q.hasAscendant("pre"),
					A = q.type != CKEDITOR.NODE_ELEMENT,
					u = 0;
					if (A) q.type == CKEDITOR.NODE_TEXT && e.test(q.getText()) && (A = 0);
					else {
						var y = q.getName();
						if (CKEDITOR.dtd.$block[y] && "false" == q.getAttribute("contenteditable")) {
							...
							b(this, a, g);
							break
						} else if (q.isBlockBoundary(this.forceBrBreak && !z && {
							br: 1
						})) {
							if ("br" == y) A = 1;
							else if (!f && !q.getChildCount() && "hr" != y) {
								g = q;
								l = q.equals(p);
								break
							}
					...
					l = (!r || A) && q.equals(p);
					if (f && !r) for (; ! q.getNext(d) && !l;) {
						y = q.getParent();
						if (y.isBlockBoundary(this.forceBrBreak && !z && {
							br: 1
						})) {
							r = 1;
							A = 0;
					...
					q = this._getNextSourceNode(q, u, p);
					if ((l = !q) || r && f) break
				}
				if (!g) {
					if (!f) return this._.docEndMarker && this._.docEndMarker.remove(),
					...
					g = g.block; ! g && q && !this.enforceRealBlocks && r[q.getName()] && f.checkStartOfBlock() && f.checkEndOfBlock() && !q.equals(f.root) ? g = q: !g || this.enforceRealBlocks && g.is(h) ? (g = this.range.document.createElement(a), f.extractContents().appendTo(g), g.trim(), f.insertNode(g), t = w = !0) : "li" != g.getName() ? f.checkStartOfBlock() && f.checkEndOfBlock() || (g = g.clone(!1), f.extractContents().appendTo(g), g.trim(), w = f.splitBlock(), t = !w.wasStartOfBlock, w = !w.wasEndOfBlock, f.insertNode(g)) : l || (this._.nextNode = g.equals(p) ? null: this._getNextSourceNode(f.getBoundaryNodes().endNode, 1, p))
				}
				t && (t = g.getPrevious()) && t.type == CKEDITOR.NODE_ELEMENT && ("br" == t.getName() ? t.remove() : t.getLast() && "br" == t.getLast().$.nodeName.toLowerCase() && t.getLast().remove());
				w && (t = g.getLast()) && t.type == CKEDITOR.NODE_ELEMENT && "br" == t.getName() && (!CKEDITOR.env.needsBrFiller || t.getPrevious(m) || t.getNext(m)) && t.remove();
				this._.nextNode || (this._.nextNode = l || g.equals(p) || !p ? null: this._getNextSourceNode(g, 1, p));
				return g
			},
	...
	function() {
		function a(a, b, c, d, e) {
			...
			for (g in b) {
				...
				for (u in K) {
					...
					E = void 0;
					y ? n = !1 : y = {};
					for (E in q)"!" == E.charAt(0) && (E = E.slice(1), C.push(E), y[E] = !0, n = !1);
					for (; E = C.pop();) q[E] = q["!" + E],
					delete q["!" + E];
					m[p] = (n ? !1 : y) || null
				}
			...
			for (d = a.length; c < d; ++c) {
				...
				for (u in J) {
					...
					y = [];
					q = void 0;
					for (q in p) - 1 < q.indexOf("*") ? y.push(new RegExp("^" + q.replace(/\*/g, ".*") + "$")) : y.push(q);
					p = y;
					p.length && (k[h] = p, m = !1)
				}
				k.nothingRequired = m;
				...
				else for (x in k = g.elements, delete g.elements, k) if (b[x]) b[x][f ? "unshift": "push"](g);
				else b[x] = [g]
			}
		}
		function f(a, c, e, g) {
		...
		function c(a, b, c) {
			if (!a.match || a.match(b)) {
				...
				if (a) {
					for (var d = !1,
					g = !0 === a,
					f = b.length; f--;) if (g || a(b[f])) b.splice(f, 1),
					d = !0;
					a = d
				} else a = !1;
		...
		function e(a, b) {
			...
			d = !0 === a,
			e;
			for (e in b) if (d || a(e)) delete b[e],
			c = !0;
			return c
		}
		function m(a, b, c) {
		...
		function l(a, b) {
			if (!a) return ! 1;
			if (!0 === a) return a;
			if ("string" == typeof a) return a = I(a),
		...
		function h(a, b) {
			if (!b) return ! 0;
			for (var c = 0,
			d; c < b.length; ++c) if (d = b[c], "string" == typeof d) {
				if (! (d in a)) return ! 1
		...
		function g(a) {
			var b, c, d, e, g = {},
			f = 1;
			for (a = I(a); b = a.match(E);)(c = b[2]) ? (d = n(c, "styles"), e = n(c, "attrs"), c = n(c, "classes")) : d = e = c = null,
			g["$" + f++] = {
				elements: b[1],
				classes: c,
				styles: d,
		...
		function t(a, b, d, e) {
			...
			if (e.doTransform && (k = a._.transformations[b.name])) {
				v(b);
			...
			if (e.doFilter) {
				a: {
					h = b.name;
					u = a._;
					a = u.allowedRules.elements[h];
					k = u.allowedRules.generic;
					h = u.disallowedRules.elements[h];
					...
					if (a || k) {
						v(b);
						if (h) for (r = 0, C = h.length; r < C; ++r) if (!1 === c(h[r], b, l)) {
							a = null;
							break a
						}
						if (u) for (r = 0, C = u.length; r < C; ++r) c(u[r], b, l);
						if (a) for (r = 0, C = a.length; r < C; ++r) f(a[r], b, l, m);
						if (k) for (r = 0, C = k.length; r < C; ++r) f(k[r], b, l, m);
						a = l
					} else a = null
				}
				if (!a || !a.valid) return d.push(b),
				1;
				C = a.validAttributes;
				var n = a.validStyles;
				k = a.validClasses;
				...
				if (!a.allAttributes) for (J in h) C[J] || (t.test(J) ? J == (z = J.replace(/^data-cke-saved-/, "")) || C[z] || (delete h[J], r = !0) : (delete h[J], r = !0));
				if (!a.allStyles || a.hadInvalidStyle) {
					for (J in E) a.allStyles || n[J] ? w.push(J + ":" + E[J]) : r = !0;
					w.length && (h.style = w.sort().join("; "))
				} else x && (h.style = x);
				if (!a.allClasses || a.hadInvalidClass) {
					for (J = 0; J < u.length; ++J)(a.allClasses || k[u[J]]) && l.push(u[J]);
					l.length && (h["class"] = l.sort().join(" "));
					m && l.length < m.split(/\s+/).length && (r = !0)
				} else m && (h["class"] = m);
				r && (g = 1);
				if (!e.skipFinalValidation && !q(b)) return d.push(b),
				1
			}
			e.toHtml && (b.name = b.name.replace(T, "cke:$1"));
			return g
		}
		function w(a) {
			var b = [],
		...
		function u(a, b, c) {
			var d = a.name;
			if (F.$empty[d] || !a.children.length)"hr" == d && "br" == b ? a.replaceWith(z()) : (a.parent && c.push({
				check: "it",
				el: a.parent
			}), a.remove());
			else if (F.$block[d] || "tr" == d) if ("br" == b) a.previous && !A(a.previous) && (b = z(), b.insertBefore(a)),
			a.next && !A(a.next) && (b = z(), b.insertAfter(a)),
			a.replaceWithChildren();
				...
				b: {
					...
					for (var g = 0,
					k = d.length,
					f; g < k; ++g) if (f = d[g], f.type == CKEDITOR.NODE_ELEMENT && !e[f.name]) {
						e = !1;
						break b
					}
					e = !0
				...
				c.push({
					...
					e = a.parent;
					for (var g = e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || "body" == e.name,
					h, u, k = d.length; 0 < k;) f = d[--k],
					g && (f.type == CKEDITOR.NODE_TEXT || f.type == CKEDITOR.NODE_ELEMENT && F.$inline[f.name]) ? (h || (h = new CKEDITOR.htmlParser.element(b), h.insertAfter(a), c.push({
						check: "parent-down",
						el: h
					})), h.add(f, 0)) : (h = null, u = F[e.name] || F.span, f.insertAfter(a), e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || f.type != CKEDITOR.NODE_ELEMENT || u[f.name] || c.push({
						check: "el-up",
						el: f
					}));
					a.remove()
		...
		function C(a, b) {
			...
			for (g in d) if ("class" == g) for (c = d[g].split(/\s+/), f = a.classes.join("|"); h = c.pop();) {
				if ( - 1 == f.indexOf(h)) return ! 1
			} else if (a.attributes[g] != d[g]) return ! 1;
			for (k in e) if (a.styles[k] != e[k]) return ! 1;
			return ! 0
		}
		function x(a, b) {
			var c, d;
		...
		CKEDITOR.filter = function(a) {
			...
			if (a instanceof CKEDITOR.editor) {
				a = this.editor = a;
				this.customConfig = !0;
				var b = a.config.allowedContent; ! 0 === b ? this.disabled = !0 : (b || (this.customConfig = !1), this.allow(b, "config", 1), this.allow(a.config.extraAllowedContent, "extra", 1), this.allow(H[a.enterMode] + " " + H[a.shiftEnterMode], "default", 1), this.disallow(a.config.disallowedContent))
		...
		CKEDITOR.filter.prototype = {
			allow: function(b, c, d) {
				...
				if ("string" == typeof b) b = g(b);
					...
					d && (d = G(d), e.classes = d["class"] ? d["class"].split(/\s+/) : null, e.requiredClasses = e.classes, delete d["class"], e.attributes = d, e.requiredAttributes = d && CKEDITOR.tools.objectKeys(d))
			...
			applyTo: function(a, b, c, d) {
				...
				a.forEach(function(a) {
					if (a.type == CKEDITOR.NODE_ELEMENT) {
						...
						if (!b || "span" != a.name || !~CKEDITOR.tools.objectKeys(a.attributes).join("|").indexOf("data-cke-")) if (f = t(e, a, g, m), f & 1) h = !0;
						...
						a: {
							...
							c = [];
							var u, l, p;
							if (k) for (l = 0; l < k.length; ++l) if ((p = d.match(k[l])) && p[0].length == d.length) {
								c = !0;
								break a
							}
							d = CKEDITOR.htmlParser.fragment.fromHtml(d);
							1 == d.children.length && (u = d.children[0]).type == CKEDITOR.NODE_ELEMENT && t(e, u, c, m);
							c = !c.length
						}
						c || g.push(a)
					}
				...
				for (var p; c = g.pop();) c.type == CKEDITOR.NODE_ELEMENT ? u(c, d, a) : c.remove();
				for (; l = a.pop();) if (c = l.el, c.parent) switch (p = F[c.parent.name] || F.span, l.check) {
				case "it":
					F.$removeEmpty[c.name] && !c.children.length ? u(c, d, a) : q(c) || u(c, d, a);
					break;
				case "el-up":
					c.parent.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || p[c.name] || u(c, d, a);
					break;
				case "parent-down":
					c.parent.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || p[c.name] || u(c.parent, d, a)
				}
				return h
			},
			...
			addContentForms: function(a) {
				if (!this.disabled && a) {
					...
					if (e) {
						for (b = 0; b < a.length; ++b) d.push(x(a[b], e));
			...
			addTransformations: function(a) {
				...
				if (!this.disabled && a) {
					...
					for (e = 0; e < a.length; ++e) {
						...
						m = void 0;
						c = [];
						for (k = 0; k < b.length; ++k) f = b[k],
						"string" == typeof f ? (f = f.split(/\s*:\s*/), h = f[0], u = null, m = f[1]) : (h = f.check, u = f.left, m = f.right),
						g || (g = f, g = g.element ? g.element: h ? h.match(/^([a-z0-9]+)/i)[0] : g.left.getDefinition().element),
						u instanceof CKEDITOR.style && (u = B(u)),
						c.push({
							check: h == g ? null: h,
							left: u,
			...
			check: function(a, b, c) {
				...
				e = d.styles,
				h = d.attributes || {},
				e && !CKEDITOR.tools.isEmpty(e) ? (e = G(e), h.style = CKEDITOR.tools.writeCssText(e, !0)) : e = {},
				e = {
					name: d.element,
					attributes: h,
				...
				if (!1 !== b && (m = this._.transformations[e.name])) {
					for (d = 0; d < m.length; ++d) y(this, e, m[d]);
					p(e)
				}
				t(this, h, u, {
					doFilter: !0,
				...
				b = 0 < u.length ? !1 : CKEDITOR.tools.objectCompare(e.attributes, h.attributes, !0) ? !0 : !1;
		...
		M = CKEDITOR.filter.transformationsTools = {
			...
			transform: function(a, b) {
				if ("string" == typeof b) a.name = b;
					...
					h;
					a.name = c.element;
					for (g in e) if ("class" == g) for (c = a.classes.join("|"), k = e[g].split(/\s+/); h = k.pop();) - 1 == c.indexOf(h) && a.classes.push(h);
	...
	function() {
		...
		return {
			load: function(b, c, e, m) {
				...
				k = [],
				g = function(a) {
					c && (l ? c.call(e, a) : c.call(e, h, k))
				};
				if (0 === d) g(!0);
				else {
					var n = function(a, b) { (b ? h: k).push(a);
						...
						if (a[b]) n(b, !0);
							...
							if (! (1 < d.length)) {
								...
								c && (CKEDITOR.env.ie && (8 >= CKEDITOR.env.version || CKEDITOR.env.ie9Compat) ? e.$.onreadystatechange = function() {
									if ("loaded" == e.$.readyState || "complete" == e.$.readyState) e.$.onreadystatechange = null,
									v(b, !0)
								}: (e.$.onload = function() {
									setTimeout(function() {
										v(b, !0)
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load: function(a, f, b) {
			...
			h = 0; h < a.length; h++) {
				var k = a[h];
				if (k) if (c[k] || e[k]) d[k] = this.get(k);
				else {
			...
			function(a, e) {
				...
				for (var g = 0; g < a.length; g++) for (var k = l[a[g]], h = 0; h < k.length; h++) {
					var m = k[h];
					d[m] = this.get(m);
					c[m] = 1
				}
			...
			l = function(b) {
				...
				function(a) {
					...
					for (d in a) {
						...
						v = n && n.requires;
						if (!f[d]) {
							if (n.icons) for (var t = n.icons.split(","), w = t.length; w--;) CKEDITOR.skin.addIcon(t[w], n.path + "icons/" + (CKEDITOR.env.hidpi && n.hidpi ? "hidpi/": "") + t[w] + ".png");
							f[d] = 1
						}
						if (v) for (v.split && (v = v.split(",")), n = 0; n < v.length; n++) m[v[n]] || b.push(v[n])
					}
					if (b.length) l.call(this, b);
					else {
						for (d in m) n = m[d],
						n.onLoad && !n.onLoad._called && (!1 === n.onLoad() && delete m[d], n.onLoad._called = 1);
						c && c.call(e || window, m)
					}
				},
				this)
	...
	function() {
		...
		function l(a) {
			...
			c = w[b] || (w[b] = {});
			c.fn ? (c.fn.call(a, a.config), CKEDITOR.getUrl(a.config.customConfig) != b && l(a) || a.fireOnce("customConfigLoaded")) : CKEDITOR.scriptLoader.queue(b,
			function() {
		...
		function n(a) {
			...
			function(c) {
				...
				for (var k in c) {
					...
					if (l && (r = l.match(f))) for (; l = r.pop();) CKEDITOR.error("editor-plugin-required", {
						plugin: l.replace(",", ""),
						requiredBy: k
					});
					u && !a.lang[k] && (u.split && (u = u.split(",")), 0 <= CKEDITOR.tools.indexOf(u, a.langCode) ? m = a.langCode: (m = a.langCode.replace(/-.*/, ""), m = m != a.langCode && 0 <= CKEDITOR.tools.indexOf(u, m) ? m: 0 <= CKEDITOR.tools.indexOf(u, "en") ? "en": u[0]), h.langEntries && h.langEntries[m] ? (a.lang[k] = h.langEntries[m], m = null) : g.push(CKEDITOR.getUrl(h.path + "lang/" + m + ".js")));
					e.push(m);
					d.push(h)
				}
				...
				function() {
					for (var c = ["beforeInit", "init", "afterInit"], g = 0; g < c.length; g++) for (var f = 0; f < d.length; f++) {
						var k = d[f];
						0 === g && e[f] && k.lang && k.langEntries && (a.lang[k.name] = k.langEntries[e[f]]);
						if (k[c[g]]) k[c[g]](a)
					}
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			...
			_attachToForm: function() {
				...
				d.is("textarea") && e && (e.on("submit", a), b(e.$.submit) && (e.$.submit = CKEDITOR.tools.override(e.$.submit,
				function(b) {
					return function() {
						a();
			...
			destroy: function(a) {
				this.fire("beforeDestroy"); ! a && v.call(this);
				this.editable(null);
			...
			setData: function(a, b, c) {
				var d = !0,
				e = b;
				b && "object" == typeof b && (c = b.internal, e = b.callback, d = !b.noSnapshot); ! c && d && this.fire("saveSnapshot");
				if (e || !c) this.once("dataReady",
				function(a) { ! c && d && this.fire("saveSnapshot");
					e && e.call(a.editor)
				});
				a = {
					dataValue: a
			...
			getSelectedHtml: function(a) {
				...
				for (var d = new CKEDITOR.dom.documentFragment,
				e, g, f, k = 0; k < c.length; k++) {
					var h = c[k],
					m = h.startContainer;
					m.getName && "tr" == m.getName() ? (e || (e = m.getAscendant("table").clone(), e.append(m.getAscendant("tbody").clone()), d.append(e), e = e.findOne("tbody")), g && g.equals(m) || (g = m, f = m.clone(), e.append(f)), f.append(h.cloneContents())) : d.append(h.cloneContents())
				}
	...
	function() {
		...
		CKEDITOR.htmlParser.prototype = {
			...
			parse: function(b) {
				for (var c, e, m = 0,
				l; c = this._.htmlPartsRegex.exec(b);) {
					e = c.index;
					if (e > m) if (m = b.substring(m, e), l) l.push(m);
					else this.onText(m);
					...
					if (l) l.push(c[0]);
						...
						if (e = e.toLowerCase(), !/="/.test(e)) {
							...
							this.onTagOpen(e, d, c); ! l && CKEDITOR.dtd.$cdata[e] && (l = [])
						}
	...
	function() {
		...
		CKEDITOR.htmlParser.node.prototype = {
			...
			getAscendant: function(a) {
				var f = "function" == typeof a ? a: "string" == typeof a ?
				function(b) {
					return b.name == a
				}: function(b) {
					return b.name in a
	...
	CKEDITOR.htmlParser.comment.prototype = CKEDITOR.tools.extend(new CKEDITOR.htmlParser.node, {
		...
		filter: function(a, f) {
			var b = this.value;
			if (! (b = a.onComment(f, b, this))) return this.remove(),
			!1;
			if ("string" != typeof b) return this.replaceWith(b),
			!1;
			this.value = b;
			return ! 0
		},
	...
	function() {
		...
		CKEDITOR.htmlParser.fragment.fromHtml = function(m, l, d) {
			function h(a) {
				var b;
				if (0 < q.length) for (var c = 0; c < q.length; c++) {
					...
					e = d.name,
					g = CKEDITOR.dtd[e],
					f = z.name && CKEDITOR.dtd[z.name];
					f && !f[e] || a && g && !g[a] && CKEDITOR.dtd[a] ? e == z.name && (n(z, z.parent, 1), c--) : (b || (k(), b = 1), d = d.clone(), d.parent = z, z = d, q.splice(c, 1), c--)
				}
			}
			function k() {
				for (; r.length;) n(r.shift(), z)
			}
			function g(a) {
			...
			function n(b, c, e) {
				c = c || z || p;
				var f = z;
				void 0 === b.previous && (v(c, b) && (z = c, w.onTagOpen(d, {}), b.returnPoint = c = z), g(b), a(b) && !b.children.length || c.add(b), "pre" == b.name && (u = !1), "textarea" == b.name && (A = !1));
				b.returnPoint ? (z = b.returnPoint, delete b.returnPoint) : z = e ? c: f
			}
			function v(a, b) {
				if ((a == p || "body" == a.name) && d && (!a.name || CKEDITOR.dtd[a.name][d])) {
					var c, e;
			...
			w.onTagOpen = function(d, e, g, m) {
				...
				if (a(e)) q.push(e);
					...
					if ("br" == d) r.push(e);
						...
						for (; ! (m = (g = z.name) ? CKEDITOR.dtd[g] || (z._.isBlockLike ? CKEDITOR.dtd.div: CKEDITOR.dtd.span) : c, e.isUnknown || z.isUnknown || m[d]);) if (z.isOptionalClose) w.onTagClose(g);
						else if (d in b && g in b) g = z.children,
						(g = g[g.length - 1]) && "li" == g.name || n(g = new CKEDITOR.htmlParser.element("li"), z),
						!e.returnPoint && (e.returnPoint = z),
						z = g;
						else if (d in CKEDITOR.dtd.$listItem && !t(d, g)) w.onTagOpen("li" == d ? "ul": "dl", {},
						0, 1);
						else if (g in f && !t(d, g)) ! e.returnPoint && (e.returnPoint = z),
						z = z.parent;
						else if (g in CKEDITOR.dtd.$inline && q.unshift(z), z.parent) n(z, z.parent, 1);
						else {
							e.isOrphan = 1;
							break
						}
			...
			w.onTagClose = function(a) {
				for (var b = q.length - 1; 0 <= b; b--) if (a == q[b].name) {
					q.splice(b, 1);
					return
				}
				for (var c = [], e = [], g = z; g != p && g.name != a;) g._.isBlockLike || e.unshift(g),
				c.push(g),
			...
			w.onText = function(a) {
				if (! (z._.hasInlineStarted && !r.length || u || A) && (a = CKEDITOR.tools.ltrim(a), 0 === a.length)) return;
				var b = z.name,
				g = b ? CKEDITOR.dtd[b] || (z._.isBlockLike ? CKEDITOR.dtd.div: CKEDITOR.dtd.span) : c;
				if (!A && !g["#"] && b in f) w.onTagOpen(e[b] || ""),
				w.onText(a);
				else {
					k();
					h();
					...
					if (v(z, a)) this.onTagOpen(d, {},
					0, 1);
					z.add(a)
				}
			};
			w.onCDATA = function(a) {
				z.add(new CKEDITOR.htmlParser.cdata(a))
			};
			w.onComment = function(a) {
				k();
				h();
				z.add(new CKEDITOR.htmlParser.comment(a))
			};
			w.parse(m);
			for (k(); z != p;) n(z, z.parent, 1);
			g(p);
			return p
		};
	...
	function() {
		...
		CKEDITOR.htmlParser.filter = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				onElement: function(a, c) {
					for (var e = [this.elementsRules["^"], this.elementsRules[c.name], this.elementsRules.$], f, l = 0; 3 > l; l++) if (f = e[l]) {
						f = f.exec(a, c, this);
						if (!1 === f) return null;
		...
		a.prototype = {
			...
			addMany: function(a, c, e) {
				for (var f = [this.findIndex(c), 0], l = 0, d = a.length; l < d; l++) f.push({
					value: a[l],
					priority: c,
					options: e
				});
			...
			findIndex: function(a) {
				for (var c = this.rules,
				e = c.length - 1; 0 <= e && a < c[e].priority;) e--;
				return e + 1
			},
			exec: function(a, c) {
				...
				g,
				n;
				for (n = 0; n < d; n++) if (e && (h = c.type, k = c.name), g = l[n], !(a.nonEditable && !g.options.applyToAll || a.nestedEditable && g.options.excludeNestedEditable)) {
					g = g.value.apply(null, f);
					if (!1 === g || e && g && (g.name != k || g.type != h)) return g;
					null != g && (f[0] = c = g)
				}
				return c
			},
			execOnName: function(a, c) {
				...
				l = f.length,
				d; c && e < l; e++) d = f[e],
				a.nonEditable && !d.options.applyToAll || a.nestedEditable && d.options.excludeNestedEditable || (c = c.replace(d.value[0], d.value[1]));
				return c
			}
		}
	} (),
	function() {
		function a(a, d) {
			...
			function f(a, d) {
				return function(e) {
					if (e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						u,
						l;
						if (h) for (k(h, 1) && f.push(h); h;) m(h) && (u = c(h)) && k(u) && ((l = c(u)) && !m(l) ? f.push(u) : (g(y).insertAfter(u), u.remove())),
						h = h.previous;
						for (h = 0; h < f.length; h++) f[h].remove();
						if (f = !a || !1 !== ("function" == typeof d ? d(e) : d)) y || CKEDITOR.env.needsBrFiller || e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT ? y || CKEDITOR.env.needsBrFiller || !(7 < document.documentMode || e.name in CKEDITOR.dtd.tr || e.name in CKEDITOR.dtd.$listItem) ? (f = b(e), f = !f || "form" == e.name && "input" == f.name) : f = !1 : f = !1;
						f && e.add(g(a))
					}
				}
			}
			function k(a, b) {
				...
				var c;
				return a.type == CKEDITOR.NODE_TEXT && (c = a.value.match(q)) && (c.index && ((new CKEDITOR.htmlParser.text(a.value.substring(0, c.index))).insertBefore(a), a.value = c[0]), !CKEDITOR.env.needsBrFiller && y && (!b || a.parent.name in n) || !y && ((c = a.previous) && "br" == c.name || !c || m(c))) ? !0 : !1
			}
			var h = {
				elements: {}
			},
			...
			u),
			E;
			for (E in n)"#" in z[E] || delete n[E];
			for (E in n) h.elements[E] = f(y, a.config.fillEmptyBlocks);
			h.root = f(y, !1);
				...
				return function(b) {
					if (b.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						if ("data-cke-bogus" in d || "data-cke-eol" in d) delete d["data-cke-bogus"];
							...
							var f = c(b); ! d && m(b.parent) ? l(b.parent, g(a)) : m(d) && f && !m(f) && g(a).insertBefore(d)
						}
					}
				}
			} (y);
		...
		function k(a) {
			...
			function(a, b, c) {
				...
				function(a, b) {
					return G.test(b) && -1 == c.indexOf("data-cke-saved-" + b) ? " data-cke-saved-" + a + " data-cke-" + CKEDITOR.rnd + "-" + a: a
				}) + "\x3e"
			})
		}
		function g(a, b) {
		...
		function p(a, b) {
			...
			for (var f = 0; f < d.length; f++) a = a.replace(d[f],
			function(a) {
				a = a.replace(g,
				function(a, b, d) {
					return c[d]
			...
			a = a.replace(g,
			function(a, b, d) {
				return "\x3c!--" + r + (b ? "{C}": "") + encodeURIComponent(c[d]).replace(/--/g, "%2D%2D") + "--\x3e"
			});
			...
			return a = a.replace(/<(title|iframe|textarea)([^>]*)>([\s\S]*?)<\/\1>/g,
			function(a, c, d, e) {
				return "\x3c" + c + d + "\x3e" + w(t(e), b) + "\x3c/" + c + "\x3e"
			})
		}
		CKEDITOR.htmlDataProcessor = function(b) {
			...
			function(a) {
				...
				c = c.replace(S, "\x3ccke:$1$2\x3e\x3c/cke:$1\x3e"),
				c = c.replace(/(<pre\b[^>]*>)(\r\n|\n)/g, "$1$2$2"),
		...
		CKEDITOR.htmlDataProcessor.prototype = {
			toHtml: function(a, b, c, d) {
				var e = this.editor,
				g, f, k, h;
				b && "object" == typeof b ? (g = b.context, c = b.fixForBody, d = b.dontFilter, f = b.filter, k = b.enterMode, h = b.protectedWhitespaces) : g = b;
	...
	function() {
		...
		CKEDITOR.htmlParser.element.prototype = CKEDITOR.tools.extend(new CKEDITOR.htmlParser.node, {
			...
			filter: function(a, b) {
				...
				for (;;) {
					l = f.name;
					if (! (d = a.onElementName(b, l))) return this.remove(),
					!1;
					f.name = d;
					if (! (f = a.onElement(b, f))) return this.remove(),
					!1;
					if (f !== this) return this.replaceWith(f),
					!1;
					if (f.name == l) break;
					if (f.type != CKEDITOR.NODE_ELEMENT) return this.replaceWith(f),
					!1;
					if (!f.name) return this.replaceWithChildren(),
					!1
				}
				l = f.attributes;
				var h, k;
				for (h in l) {
					for (d = l[h];;) if (k = a.onAttributeName(b, h)) if (k != h) delete l[h],
					h = k;
					else break;
					else {
						delete l[h];
			...
			writeHtml: function(a, b) {
				...
				a.sortAttributes && l.sort(f);
				h = 0;
				for (k = l.length; h < k; h++) d = l[h],
				a.attribute(d[0], d[1]);
			...
			getFirst: function(b) {
				...
				for (var e = 0,
				f = this.children.length; e < f; ++e) if (b(this.children[e])) return this.children[e];
				return null
			},
			getHtml: function() {
			...
			split: function(a) {
				for (var b = this.children.splice(a, this.children.length - a), f = this.clone(), l = 0; l < b.length; ++l) b[l].parent = f;
				f.children = b;
	...
	function() {
		...
		CKEDITOR.template = function(m) {
			if (a[m]) this.output = a[m];
			else {
				var l = m.replace(b, "\\$1").replace(c, "\\n").replace(e, "\\r").replace(f,
				function(a, b) {
	...
	function() {
		...
		CKEDITOR.inlineAll = function() {
			var a, f, b;
			for (b in CKEDITOR.dtd.$editable) for (var c = CKEDITOR.document.getElementsByTag(b), e = 0, m = c.count(); e < m; e++) a = c.getItem(e),
			"true" == a.getAttribute("contenteditable") && (f = {
				element: a,
				config: {}
			},
	...
	function() {
		function a(a, e, m, l) {
			...
			a = CKEDITOR.dom.element.get(a);
			if (a.getEditor()) throw 'The editor instance "' + a.getEditor().name + '" is already attached to the provided element.';
			var d = new CKEDITOR.editor(e, a, l);
			l == CKEDITOR.ELEMENT_MODE_REPLACE && (a.setStyle("visibility", "hidden"), d._.required = a.hasAttribute("required"), a.removeAttribute("required"));
			m && d.setData(m, null, !0);
			d.on("loaded",
			function() {
				b(d);
				l == CKEDITOR.ELEMENT_MODE_REPLACE && d.config.autoUpdateElement && a.$.form && d._attachToForm();
		...
		function b(a) {
			...
			l == CKEDITOR.ELEMENT_MODE_REPLACE ? (f.hide(), b.insertAfter(f)) : f.append(b);
			a.container = b;
		...
		CKEDITOR.replaceAll = function() {
			for (var a = document.getElementsByTagName("textarea"), b = 0; b < a.length; b++) {
				...
				if (l.name || l.id) {
					...
					} else if ("function" == typeof arguments[0] && (f = {},
					!1 === arguments[0](l, f))) continue;
					this.replace(l, f)
				}
			}
		};
		...
		CKEDITOR.editor.prototype.setMode = function(a, b) {
			...
			if (a != f.mode && l && l[a]) {
				...
				if (f.mode) {
					...
					"source" == f.mode && l == h && (f.fire("lockSnapshot", {
						forceUpdate: !0
					}), k = 1);
				...
				this._.modes[a](function() {
					...
					k ? f.fire("unlockSnapshot") : "wysiwyg" == a && f.fire("saveSnapshot");
					setTimeout(function() {
						f.fire("mode");
						b && b.call(f)
					},
		...
		CKEDITOR.editor.prototype.resize = function(a, b, f, l) {
			...
			l.setSize("width", a, !0);
			k && (k.style.width = "1%");
			var g = (l.$.offsetHeight || 0) - (h.$.clientHeight || 0),
	...
	function() {
		function a(a) {
			...
			k = g.getRanges()[0],
			h;
			if (CKEDITOR.env.gecko || CKEDITOR.env.ie && CKEDITOR.env.needsBrFiller) if (g = f(g, d)) g.appendBogus(),
			h = CKEDITOR.env.ie;
			l(b, d.block, e) && k.collapsed && !k.getCommonAncestor().isReadOnly() && (d = k.clone(), d.enlarge(CKEDITOR.ENLARGE_BLOCK_CONTENTS), e = new CKEDITOR.dom.walker(d), e.guard = function(a) {
				return ! c(a) || a.type == CKEDITOR.NODE_COMMENT || a.isReadOnly()
			},
			!e.checkForward() || d.checkStartOfBlock() && d.checkEndOfBlock()) && (b = k.fixBlock(!0, b.activeEnterMode == CKEDITOR.ENTER_DIV ? "div": "p"), CKEDITOR.env.needsBrFiller || (b = b.getFirst(c)) && b.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(b.getText()).match(/^(?:&nbsp;|\xa0)$/) && b.remove(), h = 1, a.cancel());
			h && k.select()
		}
		function f(a, b) {
		...
		function m(a) {
			...
			if (e.startPath().contains(g)) {
				...
				f.collapse(),
				f.setEndAt(d, CKEDITOR.POSITION_AFTER_END),
				a = new CKEDITOR.dom.walker(f),
				a.guard = b(!0),
				d = !1,
				a.checkForward(),
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				focus: function() {
					...
					try { ! CKEDITOR.env.ie || CKEDITOR.env.edge && 14 < CKEDITOR.env.version || !this.getDocument().equals(CKEDITOR.document) ? this.$.focus() : this.$.setActive()
					} catch(b) {
				...
				insertElementIntoRange: function(a, b) {
					...
					b.startContainer.type == CKEDITOR.NODE_ELEMENT && (b.startContainer.is({
						...
						thead: 1,
						tfoot: 1
					}) ? q(b) : b.startContainer.is(CKEDITOR.dtd.$list) && r(b));
					var f, k;
					if (g) for (; (f = b.getCommonAncestor(0, 1)) && (k = CKEDITOR.dtd[f.getName()]) && (!k || !k[e]);) f.getName() in CKEDITOR.dtd.span ? b.splitElement(f) : b.checkStartOfBlock() && b.checkEndOfBlock() ? (b.setStartBefore(f), b.collapse(!0), f.remove()) : b.splitBlock(d == CKEDITOR.ENTER_DIV ? "div": "p", c.editable());
					b.insertNode(a);
					return ! 0
				},
				...
				setup: function() {
					...
					function() {
						var b = this.getData();
						this.is("textarea") || !1 !== a.config.ignoreEmptyParagraph && (b = b.replace(w,
						function(a, b) {
							return b
						}));
						a.setData(b, null, 1)
					...
					null, null, -1);
					if (CKEDITOR.env.webkit) this.on("scroll",
					function() {
					...
					if (!this.is("textarea")) {
						...
						this.attachListener(this, "dblclick",
						function(b) {
						...
						function(b) { (b = b.data.getTarget()) && b.is("img") && a.getSelection().selectElement(b)
						});
						CKEDITOR.env.gecko && this.attachListener(this, "mouseup",
						function(b) {
							if (2 == b.data.$.button && (b = b.data.getTarget(), !b.getOuterHtml().replace(w, ""))) {
								var c = a.createRange();
						...
						CKEDITOR.env.webkit && this.attachListener(a, "key",
						function(b) {
							if (a.readOnly) return ! 0;
							...
							if (b in n) {
								...
								if (d.collapsed) a: {
									...
									if (e && d[c ? "checkStartOfBlock": "checkEndOfBlock"]() && d.moveToClosestEditablePosition(e, !c) && d.collapsed) {
										...
										if (!d || d && d.contains(e)) b = void 0;
											...
											a.fire("saveSnapshot");
											var f; (f = (c ? d: e).getBogus()) && f.remove();
											f = a.getSelection();
											g = f.createBookmarks(); (c ? e: d).moveChildren(c ? d: e, !1);
								...
								f = b.block,
								d = c.endPath().block,
								f && d && !f.equals(d) ? (a.fire("saveSnapshot"), (e = f.getBogus()) && e.remove(), c.enlarge(CKEDITOR.ENLARGE_INLINE), c.deleteContents(), d.getParent() && (d.moveChildren(f, !1), b.lastElement.mergeSiblings(), k(f, d, !0)), c = a.getSelection().getRanges()[0], c.collapse(1), c.optimize(), "" === c.startContainer.getHtml() && c.startContainer.appendBogus(), c.select(), b = !0) : b = !1;
								if (!b) return;
		...
		p = function() {
			...
			function b(c, d) {
				...
				e = d.range.startPath();
				for (var l = m[l.getName()], n = 0, r = c.getChildren(), q = r.count(), p = -1, w = -1, C = 0, t = e.contains(m.$list); n < q; ++n) e = r.getItem(n),
				a(e) ? (f = e.getName(), t && f in CKEDITOR.dtd.$list ? h = h.concat(b(e, d)) : (k = !!l[f], "br" != f || !e.data("cke-eol") || n && n != q - 1 || (C = (g = n ? h[n - 1].node: r.getItem(n + 1)) && (!a(g) || !g.is("br")), g = g && a(g) && m.$block[g.getName()]), -1 != p || k || (p = n), k || (w = n), h.push({
					isElement: 1,
					isLineBreak: C,
					...
					node: e,
					allowed: 1
				}); - 1 < p && (h[p].firstNotAllowed = 1); - 1 < w && (h[w].lastNotAllowed = 1);
				return h
			}
			function e(b, c) {
				var d = [],
				...
				for (n && d.push(" "); h < f; h++) k = g.getItem(h),
				a(k) && !k.is(l) ? d = d.concat(e(k, c)) : d.push(k);
				n && d.push(" ");
				return d
			}
			function g(b) {
			...
			function k(b, c, d, e) {
				...
				f,
				h;
				g.setEndAt(c, CKEDITOR.POSITION_BEFORE_END); (f = (new CKEDITOR.dom.walker(g)).next()) && a(f) && n[f.getName()] && (h = f.getPrevious()) && a(h) && !h.getParent().equals(b.startContainer) && d.contains(h) && e.contains(f) && f.isIdentical(h) && (f.moveChildren(h), f.remove(), k(b, c, d, e))
			}
			function h(b, c) {
				function d(b, c) {
			...
			m.$inline);
			delete q.br;
			return function(n, E, p, w) {
				var t = n.editor,
				z = !1;
				"unfiltered_html" == E && (E = "html", z = !0);
				if (!w.checkReadOnly()) {
					var v = (new CKEDITOR.dom.elementPath(w.startContainer, w.root)).blockLimit || w.root;
					n = {
						type: E,
						dontFilter: z,
						editable: n,
					...
					w = n.mergeCandidates;
					var A, I;
					"text" == n.type && E.shrink(CKEDITOR.SHRINK_ELEMENT, !0, !1) && (A = CKEDITOR.dom.element.createFromHtml("\x3cspan\x3e\x26nbsp;\x3c/span\x3e", E.document), E.insertNode(A), E.setStartAfter(A));
					z = new CKEDITOR.dom.elementPath(E.startContainer);
					n.endPath = v = new CKEDITOR.dom.elementPath(E.endContainer);
					if (!E.collapsed) {
						var t = v.block || v.blockLimit,
						ga = E.getCommonAncestor();
						t && !t.equals(ga) && !t.contains(ga) && E.checkEndOfBlock() && n.zombies.push(t);
						E.deleteContents()
					}
					for (; (I = g(E)) && a(I) && I.isBlockBoundary() && z.contains(I);) E.moveToPosition(I, CKEDITOR.POSITION_BEFORE_END);
					k(E, n.blockLimit, z, v);
					A && (E.setEndBefore(A), E.collapse(), A.remove());
					A = E.startPath();
					if (t = A.contains(f, !1, 1)) E.splitElement(t),
					n.inlineStylesRoot = t,
					n.inlineStylesPeak = A.lastElement;
					A = E.createBookmark(); (t = A.startNode.getPrevious(c)) && a(t) && f(t) && w.push(t); (t = A.startNode.getNext(c)) && a(t) && f(t) && w.push(t);
					for (t = A.startNode; (t = t.getParent()) && f(t);) w.push(t);
					E.moveToBookmark(A);
					if (A = p) {
						A = n.range;
						if ("text" == n.type && n.inlineStylesRoot) {
							...
							for (w = n.inlineStylesRoot.getParent(); ! I.equals(w);) E = E.appendTo(I.clone()),
							I = I.getParent();
							p = E.getOuterHtml().split("{cke-peak}").join(p)
						}
						...
						I = A.document.createElement("body");
						I.setHtml(p);
						N && (I.getFirst().remove(), I.getLast().remove());
						if ((N = A.startPath().block) && (1 != N.getChildCount() || !N.getBogus())) a: {
							var P;
							if (1 == I.getChildCount() && a(P = I.getFirst()) && P.is(r) && !P.hasAttribute("contenteditable")) {
								N = P.getElementsByTag("*");
								A = 0;
								for (w = N.count(); A < w; A++) if (E = N.getItem(A), !E.is(q)) break a;
					...
					if (A) {
						...
						aa,
						v = z.getPosition(E),
						ga = !!z.getCommonAncestor(E) && v != CKEDITOR.POSITION_IDENTICAL && !(v & CKEDITOR.POSITION_CONTAINS + CKEDITOR.POSITION_IS_CONTAINED);
						E = b(n.dataWrapper, n);
						for (h(E, P); w < E.length; w++) {
							v = E[w];
							if (L = v.isLineBreak) {
								L = P;
								U = I;
								var X = void 0,
								ba = void 0;
								v.hasBlockSibling ? L = 1 : (X = L.startContainer.getAscendant(m.$block, 1)) && X.is({
									div: 1,
									p: 1
								}) ? (ba = X.getPosition(U), ba == CKEDITOR.POSITION_IDENTICAL || ba == CKEDITOR.POSITION_CONTAINS ? L = 0 : (U = L.splitElement(X), L.moveToPosition(U, CKEDITOR.POSITION_AFTER_START), L = 1)) : L = 0
							}
							if (L) p = 0 < w;
							else {
								L = P.startPath(); ! v.isBlock && l(n.editor, L.block, L.blockLimit) && (Y = d(n.editor)) && (Y = A.createElement(Y), Y.appendBogus(), P.insertNode(Y), CKEDITOR.env.needsBrFiller && (R = Y.getBogus()) && R.remove(), P.moveToPosition(Y, CKEDITOR.POSITION_BEFORE_END));
								if ((L = P.startPath().block) && !L.equals(Q)) {
									if (R = L.getBogus()) R.remove(),
									N.push(L);
									Q = L
								}
								v.firstNotAllowed && (t = 1);
								if (t && v.isElement) {
									L = P.startContainer;
									for (U = null; L && !m[L.getName()][v.name];) {
										if (L.equals(I)) {
											L = null;
											break
										}
									...
									if (L) U && (Z = P.splitElement(U), n.zombies.push(Z), n.zombies.push(U));
										...
										L = w == E.length - 1;
										U = e(v.node, U);
										for (var X = [], ba = U.length, ea = 0, ha = void 0, ia = 0, fa = -1; ea < ba; ea++) ha = U[ea],
										" " == ha ? (ia || aa && !ea || (X.push(new CKEDITOR.dom.text(" ")), fa = X.length), ia = 1) : (X.push(ha), ia = 0);
										L && fa == X.length && X.pop();
										aa = X
									}
								}
								...
								v.lastNotAllowed && w < E.length - 1 && ((Z = ga ? z: Z) && P.setEndAt(Z, CKEDITOR.POSITION_AFTER_START), t = 0);
								P.collapse()
							}
						}
						1 != E.length ? R = !1 : (R = E[0], R = R.isElement && "false" == R.node.getAttribute("contenteditable"));
						R && (p = !0, L = E[0].node, P.setStartAt(L, CKEDITOR.POSITION_BEFORE_START), P.setEndAt(L, CKEDITOR.POSITION_AFTER_END));
						n.dontMoveCaret = p;
					...
					if (Z) for (; Q = Z.pop();) CKEDITOR.env.needsBrFiller ? Q.appendBogus() : Q.append(R.document.createText(" "));
		...
		q = function() {
			...
			function c(a) {
				var b = a.count(),
				d;
				for (b; 0 < b--;) d = a.getItem(b),
				CKEDITOR.tools.trim(d.getHtml()) || (d.appendBogus(), CKEDITOR.env.ie && 9 > CKEDITOR.env.version && d.getChildCount() && d.getFirst().remove())
			}
			return function(d) {
		...
		z = {
			...
			bogus: {
				exclude: function(a) {
					...
					c = b.startNode,
					b = b.endNode; ! b || !t(b) || c && c.equals(b) || a.range.setEndBefore(b)
				}
			},
			tree: {
				...
				rebuildFragment: function(a, b, c, d) {
					for (var e; c && !c.equals(b) && d(c);) e = c.clone(0, 1),
		...
		A = function() {
			...
			function b(c, d, e) {
				a(d);
				a(e, 1);
				for (var g; g = e.getNext();) g.insertAfter(d),
				d = g;
				v(c) && c.remove()
			}
			function c(a, b) {
				...
				table: function() {
					function a(c) {
						...
						f.guard = function(a, f) {
							...
							if (f && k && a.equals(k)) g = c.clone(),
							g.setEndAt(k, CKEDITOR.POSITION_BEFORE_END),
							e.push(g);
							else if (!f && h && a.equals(h)) g = c.clone(),
							g.setStartAt(h, CKEDITOR.POSITION_AFTER_START),
							e.push(g);
							else {
					...
					return {
						...
						detectRanges: function(e, g) {
							...
							m = m.contains("table");
							l = l.contains("table");
							if (m || l) m && l && b(m, l) ? (e.tableSurroundingRange = k, k.setStartAt(m, CKEDITOR.POSITION_AFTER_END), k.setEndAt(l, CKEDITOR.POSITION_BEFORE_START), k = f.clone(), k.setEndAt(m, CKEDITOR.POSITION_AFTER_END), h = f.clone(), h.setStartAt(l, CKEDITOR.POSITION_BEFORE_START), h = a(k).concat(a(h))) : m ? l || (e.tableSurroundingRange = k, k.setStartAt(m, CKEDITOR.POSITION_AFTER_END), f.setEndAt(m, CKEDITOR.POSITION_AFTER_END)) : (e.tableSurroundingRange = k, k.setEndAt(l, CKEDITOR.POSITION_BEFORE_START), f.setStartAt(l, CKEDITOR.POSITION_AFTER_START)),
							e.tableContentsRanges = h ? h: a(f)
						},
	...
	function() {
		...
		function b() {
			p = null;
			q && (CKEDITOR.tools.setTimeout(a, 0, this), q = !1)
		}
		function c(a) {
		...
		function g(a) {
			for (var b = 0; b < a.length; b++) {
				...
				if (!c.collapsed) {
					...
					e; d && !((e = d.type == CKEDITOR.NODE_ELEMENT) && d.is("body") || !d.isReadOnly());) e && "false" == d.getAttribute("contentEditable") && c.setStartAfter(d),
					d = d.getParent();
					d = c.startContainer;
					e = c.endContainer;
					var g = c.startOffset,
		...
		p,
		q,
		r = CKEDITOR.dom.walker.invisible(1),
		z = function() {
			function a(b) {
				return function(a) {
			...
			function b(a) {
				return function(b) {
					var c = b.editor,
					d = c.createRange(),
					e; (e = d.moveToClosestEditablePosition(b.selected, a)) || (e = d.moveToClosestEditablePosition(b.selected, !a));
		...
		function(b) {
			...
			function() {
				...
				function(a) {
					a.removeListener();
					0 !== q && (a = d.getSelection().getNative()) && a.isCollapsed && a.anchorNode == h.$ && (a = d.createRange(), a.moveToElementEditStart(h), a.select())
				},
				null, null, -2);
				h.attachListener(h, CKEDITOR.env.webkit ? "DOMFocusIn": "focus",
				function() {
					q && CKEDITOR.env.webkit && (q = d._.previousActive && d._.previousActive.equals(e.getActive())) && null != d._.previousScrollTop && d._.previousScrollTop != h.$.scrollTop && (h.$.scrollTop = d._.previousScrollTop);
					d.unlockSelection(q);
					q = 0
				},
				null, null, -1);
				h.attachListener(h, "mousedown",
				function() {
					q = 0
				});
				if (CKEDITOR.env.ie || u) n ? h.attachListener(h, "beforedeactivate", b, null, null, -1) : h.attachListener(d, "selectionCheck", b, null, null, -1),
				h.attachListener(h, CKEDITOR.env.webkit ? "DOMFocusOut": "blur",
				function() {
					d.lockSelection(E);
					q = 1
				},
				...
				if (CKEDITOR.env.ie && !u) {
					...
					h.attachListener(h, "mousedown",
					function(a) {
						2 == a.data.$.button && ((a = d.document.getSelection()) && a.getType() != CKEDITOR.SELECTION_NONE || (p = d.window.getScrollPosition()))
					});
					h.attachListener(h, "mouseup",
					function(a) {
						2 == a.data.$.button && p && (d.document.$.documentElement.scrollLeft = p.x, d.document.$.documentElement.scrollTop = p.y);
						p = null
					});
					if ("BackCompat" != e.$.compatMode) {
							...
							function(a) {
								function b(a) {
									...
									if (w) {
										...
										w.setEndPoint(0 > t.compareEndPoints("StartToStart", c) ? "EndToEnd": "StartToStart", c);
										w.select()
									}
								}
								function c() {
									r.removeListener("mousemove", b);
									g.removeListener("mouseup", c);
									r.removeListener("mouseup", c);
									w.select()
								}
								a = a.data;
								if (a.getTarget().is("html") && a.$.y < r.$.clientHeight && a.$.x < r.$.clientWidth) {
									w = m.$.createTextRange();
									try {
										w.moveToPoint(a.$.clientX, a.$.clientY)
									} catch(d) {}
									t = w.duplicate();
									r.on("mousemove", b);
									g.on("mouseup", c);
						...
						if (7 < CKEDITOR.env.version && 11 > CKEDITOR.env.version) r.on("mousedown",
						function(a) {
							a.data.getTarget().is("html") && (g.on("mouseup", c), r.on("mouseup", c))
						})
					}
				}
				...
				if (u && (CKEDITOR.env.webkit || CKEDITOR.env.gecko)) {
					...
					h.attachListener(e.getDocumentElement(), "mouseup",
					function() {
						z && f.call(d);
						z = 0
					})
				} else h.attachListener(CKEDITOR.env.ie ? h: e.getDocumentElement(), "mouseup", f, d);
				CKEDITOR.env.webkit && h.attachListener(e, "keydown",
				function(a) {
		...
		if (CKEDITOR.env.webkit) CKEDITOR.on("instanceReady",
		function(a) {
			var b = a.editor;
		...
		CKEDITOR.editor.prototype.selectionChange = function(b) { (b ? a: f).call(this)
		};
		...
		CKEDITOR.dom.selection.prototype = {
			...
			function() {
				...
				if (!c || !c.rangeCount) b = CKEDITOR.SELECTION_NONE;
					...
					d = c.startContainer;
					d == c.endContainer && 1 == d.nodeType && 1 == c.endOffset - c.startOffset && A[d.childNodes[c.startOffset].nodeName.toLowerCase()] && (b = CKEDITOR.SELECTION_ELEMENT)
				}
				return a.type = b
			},
			getRanges: function() {
				...
				function() {
					...
					var b = function(b, c) {
						...
						for (var e = d.children,
						g, f, h = b.duplicate(), k = 0, l = e.length - 1, m = -1, n, r; k <= l;) if (m = Math.floor((k + l) / 2), g = e[m], h.moveToElementText(g), n = h.compareEndPoints("StartToStart", b), 0 < n) l = m - 1;
						else if (0 > n) k = m + 1;
						else return {
							container: d,
							offset: a(g)
						...
						if ( - 1 == m || m == e.length - 1 && 0 > n) {
							...
							for (d = e.length; 0 < h && 0 < d;) f = e[--d],
							f.nodeType == CKEDITOR.NODE_TEXT && (r = f, h -= f.nodeValue.length);
							return {
								container: r,
								offset: -h
							}
						...
						for (; 0 < h;) try {
							f = g[0 < n ? "previousSibling": "nextSibling"],
							f.nodeType == CKEDITOR.NODE_TEXT && (h -= f.nodeValue.length, r = f),
							g = f
						} catch(q) {
							return {
								container: d,
						...
						return {
							container: r,
							offset: 0 < n ? -h: r.nodeValue.length + h
						}
					};
					return function() {
			...
			selectRanges: function(a) {
				...
				this.reset();
				if (b) for (var b = this.root,
				c, d = 0; d < a.length; ++d) c = a[d],
				c.endContainer.equals(b) && (c.endOffset = Math.min(c.endOffset, b.getChildCount()));
				if (a.length) if (this.isLocked) {
					...
					a: {
						var h, k;
						if (1 == a.length && !(k = a[0]).collapsed && (f = k.getEnclosedNode()) && f.type == CKEDITOR.NODE_ELEMENT && (k = k.clone(), k.shrink(CKEDITOR.SHRINK_ELEMENT, !0), (h = k.getEnclosedNode()) && h.type == CKEDITOR.NODE_ELEMENT && (f = h), "false" == f.getAttribute("contenteditable"))) break a;
						f = void 0
					}
					if (f) this.fake(f);
					else {
						if (n) {
							...
							a = f.collapsed;
							var r, q, p;
							if ((c = f.getEnclosedNode()) && c.type == CKEDITOR.NODE_ELEMENT && c.getName() in A && (!c.is("a") || !c.getText())) try {
								p = c.$.createControlRange();
								p.addElement(c.$);
								p.select();
								return
							} catch(w) {}
							if (f.startContainer.type == CKEDITOR.NODE_ELEMENT && f.startContainer.getName() in b || f.endContainer.type == CKEDITOR.NODE_ELEMENT && f.endContainer.getName() in b) f.shrink(CKEDITOR.NODE_ELEMENT, !0),
							a = f.collapsed;
							p = f.createBookmark();
							b = p.startNode;
							a || (g = p.endNode);
							p = f.document.$.body.createTextRange();
							p.moveToElementText(b.$);
							p.moveStart("character", 1);
							g ? (h = f.document.$.body.createTextRange(), h.moveToElementText(g.$), p.setEndPoint("EndToEnd", h), p.moveEnd("character", -1)) : (r = b.getNext(k), q = b.hasAscendant("pre"), r = !(r && r.getText && r.getText().match(h)) && (q || !b.hasPrevious() || b.getPrevious().is && b.getPrevious().is("br")), q = f.document.createElement("span"), q.setHtml("\x26#65279;"), q.insertBefore(b), r && f.document.createText("﻿").insertBefore(b));
							f.setStartBefore(b);
							b.remove();
							a ? (r ? (p.moveStart("character", -1), p.select(), f.document.$.selection.clear()) : p.select(), f.moveToPosition(q, CKEDITOR.POSITION_BEFORE_START), q.remove()) : (f.setEndBefore(g), g.remove(), p.select())
						} else {
							g = this.getNative();
							if (!g) return;
							this.removeAllRanges();
							for (p = 0; p < a.length; p++) {
								if (p < a.length - 1 && (r = a[p], q = a[p + 1], h = r.clone(), h.setStart(r.endContainer, r.endOffset), h.setEnd(q.startContainer, q.startOffset), !h.collapsed && (h.shrink(CKEDITOR.NODE_ELEMENT, !0), f = h.getCommonAncestor(), h = h.getEnclosedNode(), f.isReadOnly() || h && h.isReadOnly()))) {
								...
								f.collapsed && CKEDITOR.env.webkit && e(f) && (h = m(this.root), f.insertNode(h), (r = h.getNext()) && !h.getPrevious() && r.type == CKEDITOR.NODE_ELEMENT && "br" == r.getName() ? (l(this.root), f.moveToPosition(r, CKEDITOR.POSITION_BEFORE_START)) : f.moveToPosition(h, CKEDITOR.POSITION_AFTER_END));
								q.setStart(f.startContainer.$, f.startOffset);
								try {
									q.setEnd(f.endContainer.$, f.endOffset)
								} catch(E) {
									if (0 <= E.toString().indexOf("NS_ERROR_ILLEGAL_VALUE")) f.collapse(1),
									q.setEnd(f.endContainer.$, f.endOffset);
									else throw E;
								}
								g.addRange(q)
	...
	function() {
		...
		function b(c) {
			var d = c.document;
			if (c.collapsed) d = r(this, d),
			c.insertNode(d),
			c.moveToPosition(d, CKEDITOR.POSITION_BEFORE_END);
			else {
				var g = this.element,
				h = this._.definition,
				...
				if (!l) {
					...
					l = a(p, v),
					v = a(t, v);
					l && (z = l.getNextSourceNode(!0));
					v && (t = v)
				}
				for (z.getPosition(t) == CKEDITOR.POSITION_FOLLOWING && (z = 0); z;) {
					l = !1;
					if (z.equals(t)) z = null,
					l = !0;
					else {
						var A = z.type == CKEDITOR.NODE_ELEMENT ? z.getName() : null,
						v = A && "false" == z.getAttribute("contentEditable"),
						y = A && z.getAttribute("data-nostyle");
						if (A && z.data("cke-bookmark")) {
							z = z.getNextSourceNode(!0);
							continue
						}
						if (v && m && CKEDITOR.dtd.$block[A]) for (var C = z,
						x = e(C), B = void 0, D = x.length, ea = 0, C = D && new CKEDITOR.dom.range(C.getDocument()); ea < D; ++ea) {
							var B = x[ea],
							ha = CKEDITOR.filter.instances[B.data("cke-filter")];
							if (ha ? ha.check(this) : 1) C.selectNodeContents(B),
							b.call(this, C)
						}
						x = A ? !n[A] || y ? 0 : v && !m ? 0 : f(z, t, h, K) : 1;
						if (x) if (B = z.getParent(), x = h, D = g, ea = k, !B || !(B.getDtd() || CKEDITOR.dtd.span)[D] && !ea || x.parentRule && !x.parentRule(B)) l = !0;
						else {
							if (u || A && CKEDITOR.dtd.$removeEmpty[A] && (z.getPosition(t) | K) != K || (u = c.clone(), u.setStartBefore(z)), A = z.type, A == CKEDITOR.NODE_TEXT || v || A == CKEDITOR.NODE_ELEMENT && !z.getChildCount()) {
								for (var A = z,
								H; (l = !A.getNext(I)) && (H = A.getParent(), n[H.getName()]) && f(H, p, h, J);) A = H;
								u.setEndAfter(A)
							}
						} else l = !0;
						z = z.getNextSourceNode(y || v)
					}
					if (l && u && !u.collapsed) {
						for (var l = r(this, d), v = l.hasAttributes(), y = u.getCommonAncestor(), A = {},
						x = {},
						B = {},
						D = {},
						fa, W, F; l && y;) {
							if (y.getName() == g) {
								for (fa in h.attributes) ! D[fa] && (F = y.getAttribute(W)) && (l.getAttribute(fa) == F ? x[fa] = 1 : D[fa] = 1);
								for (W in h.styles) ! B[W] && (F = y.getStyle(W)) && (l.getStyle(W) == F ? A[W] = 1 : B[W] = 1)
							}
							y = y.getParent()
						}
						for (fa in x) l.removeAttribute(fa);
						for (W in A) l.removeStyle(W);
						v && !l.hasAttributes() && (l = null);
						l ? (u.extractContents().appendTo(l), u.insertNode(l), w.call(this, l), l.mergeSiblings(), CKEDITOR.env.ie || l.$.normalize()) : (l = new CKEDITOR.dom.element("span"), u.extractContents().appendTo(l), u.insertNode(l), w.call(this, l), l.remove(!0));
						u = null
					}
				}
				c.moveToBookmark(q);
		...
		function c(a) {
			...
			if (a.collapsed) {
				for (var e = new CKEDITOR.dom.elementPath(d.getParent(), a.root), g, f = 0, h; f < e.elements.length && (h = e.elements[f]) && h != e.block && h != e.blockLimit; f++) if (this.checkElementRemovable(h)) {
					var k;
					a.collapsed && (a.checkBoundaryOfElement(h, CKEDITOR.END) || (k = a.checkBoundaryOfElement(h, CKEDITOR.START))) ? (g = h, g.match = k ? "start": "end") : (h.mergeSiblings(), h.is(this.element) ? t.call(this, h) : p(h, u(this)[h.getName()]))
				}
				if (g) {
					h = d;
					for (f = 0;; f++) {
				...
				m = this;
				b();
				for (e = d; ! e.equals(l);) g = e.getNextSourceNode(),
				e.type == CKEDITOR.NODE_ELEMENT && this.checkElementRemovable(e) && (e.getName() == this.element ? t.call(this, e) : p(e, u(this)[e.getName()]), g.type == CKEDITOR.NODE_ELEMENT && g.contains(d) && (b(), g = d.getNext())),
				e = g
			}
			a.moveToBookmark(c);
		...
		function d(a) {
			...
			g; d = c.getNextParagraph();) ! d.isReadOnly() && (c.activeFilter ? c.activeFilter.check(this) : 1) && (g = r(this, e, d), k(d, g));
			a.moveToBookmark(b)
		}
		function h(a) {
			...
			for (var d, e; d = c.getNextParagraph();) this.checkElementRemovable(d) && (d.is("pre") ? ((e = this._.enterMode == CKEDITOR.ENTER_BR ? null: a.document.createElement(this._.enterMode == CKEDITOR.ENTER_P ? "p": "div")) && d.copyAttributes(e), k(d, e)) : t.call(this, d));
			a.moveToBookmark(b)
		}
		function k(a, b) {
			var c = !b;
			...
			if (d && !e) {
				...
				b = e
			} else f ? b = v(c ? [a.getHtml()] : g(a), b) : a.moveChildren(b);
			b.replace(a);
			if (d) {
				var c = b,
				k; (k = c.getPrevious(H)) && k.type == CKEDITOR.NODE_ELEMENT && k.is("pre") && (d = n(k.getHtml(), /\n$/, "") + "\n\n" + n(c.getHtml(), /^\n/, ""), CKEDITOR.env.ie ? c.$.outerHTML = "\x3cpre\x3e" + d + "\x3c/pre\x3e": c.setHtml(d), k.remove())
			} else c && q(b)
		}
		function g(a) {
			var b = [];
		...
		function w(a) {
			for (var b = u(this), c = a.getElementsByTag(this.element), d, e = c.count(); 0 <= --e;) d = c.getItem(e),
			d.isReadOnly() || t.call(this, d, !0);
			for (var g in b) if (g != this.element) for (c = a.getElementsByTag(g), e = c.count() - 1; 0 <= e; e--) d = c.getItem(e),
			d.isReadOnly() || p(d, b[g])
		}
		function p(a, b, c) {
			if (b = b && b.attributes) for (var d = 0; d < b.length; d++) {
				...
				if (g = a.getAttribute(e)) {
					var f = b[d][1]; (null === f || f.test && f.test(g) || "string" == typeof f && g == f) && a.removeAttribute(e)
				}
			}
			c || q(a)
		}
		...
		function A(a, b) {
			for (var c in a) a[c] = a[c].replace(G,
			function(a, c) {
				return b[c]
			})
		}
		...
		CKEDITOR.style = function(a, b) {
			...
			this.type = a.type || (B[c] ? CKEDITOR.STYLE_BLOCK: D[c] ? CKEDITOR.STYLE_OBJECT: CKEDITOR.STYLE_INLINE);
		...
		CKEDITOR.style.prototype = {
			...
			checkElementMatch: function(a, b) {
				...
				if ("string" == typeof this.element ? d == this.element: d in this.element) {
					...
					if (d = c._AC) c = d;
						...
						e = 0,
						g = c.attributes;
						if (g) for (var f in g) e++,
						d[f] = g[f];
						if (f = CKEDITOR.style.getStyleText(c)) d.style || e++,
						d.style = f;
						d._length = e;
						c = c._AC = d
					}
					if (c._length) {
						for (var h in c) if ("_length" != h) if (d = a.getAttribute(h) || "", "style" == h ? C(c[h], d) : c[h] == d) {
							if (!b) return ! 0
						} else if (b) return ! 1;
						if (b) return ! 0
					} else return ! 0
			...
			buildPreview: function(a) {
				...
				var c = ["\x3c", d],
				e = b.attributes;
				if (e) for (var g in e) c.push(" ", g, '\x3d"', e[g], '"'); (e = CKEDITOR.style.getStyleText(b)) && c.push(' style\x3d"', e, '"');
	...
	CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
		...
		getStylesSet: function(a) {
			if (this._.stylesDefinitions) a(this._.stylesDefinitions);
				...
				if (!1 === b) a(null);
				else if (b instanceof Array) f._.stylesDefinitions = b,
				a(b);
				else {
					b || (b = "default");
					var b = b.split(":"),
	...
	function() {
		...
		contains: function(a, f, b) {
			...
			for (f = 0; f < m; f++) if (c(e[f])) return e[f];
			return null
		},
		...
		var f = {
				...
				return {
					getNextRange: function(l) {
						...
						if (d && 1 < a.length) {
							...
							for (d.moveToBookmark(e.shift()); k--;) h = a[++f],
							h.moveToBookmark(e.shift()),
			...
			createBookmarks: function(b) {
				for (var c = [], e, f = 0; f < this.length; f++) {
					...
					for (var l = f + 1; l < this.length; l++) this[l] = a(e, this[l]),
					this[l] = a(e, this[l], !0)
				}
				return c
			},
	...
	function() {
		...
		if (a = CKEDITOR._.pending) for (delete CKEDITOR._.pending, f = 0; f < a.length; f++) CKEDITOR.editor.prototype.constructor.apply(a[f][0], a[f][1]),
		CKEDITOR.add(a[f][0])
		...
		onLoad: function() {
			...
			CKEDITOR.tools.extend(CKEDITOR.ui.dialog, {
				labeledElement: function(b, c, d, e) {
					if (! (4 > arguments.length)) {
						...
						CKEDITOR.ui.dialog.uiElement.call(this, b, c, d, "div", null, l,
						function() {
							var a = [],
				...
				textInput: function(b, c, e) {
					if (! (3 > arguments.length)) {
						...
						function() {
							...
							function(a) {
								13 == a.data.getKeystroke() && (w = !0)
							});
							m.getInputElement().on("keyup",
							function(a) {
								13 == a.data.getKeystroke() && w && (b.getButton("ok") && setTimeout(function() {
					...
					if (! (3 > arguments.length)) {
						...
						function() {
							for (var a = [], d = [], g = (c.id ? c.id: CKEDITOR.tools.getNextId()) + "_radio", m = 0; m < c.items.length; m++) {
								var r = c.items[m],
								z = void 0 !== r[2] ? r[2] : r[0],
								A = void 0 !== r[1] ? r[1] : r[0],
								...
								x = [];
								f._["default"] == A && (C.checked = "checked");
								l(u);
								l(z);
								"undefined" != typeof u.inputStyle && (u.style = u.inputStyle);
								u.keyboardFocusable = !0;
								e.push(new CKEDITOR.ui.dialog.uiElement(b, u, x, "input", null, C));
								x.push(" ");
				...
				button: function(b, c, d) {
					if (arguments.length) {
						...
						'\x3cspan id\x3d"' + l + '" class\x3d"cke_dialog_ui_button"\x3e' + CKEDITOR.tools.htmlEncode(c.label) + "\x3c/span\x3e")
					}
				},
				select: function(b, c, d) {
					if (! (3 > arguments.length)) {
						...
						function() {
							...
							l(a);
							for (var m = 0,
							r; m < c.items.length && (r = c.items[m]); m++) f.push('\x3coption value\x3d"', CKEDITOR.tools.htmlEncode(void 0 !== r[1] ? r[1] : r[0]).replace(/"/g, "\x26quot;"), '" /\x3e ', CKEDITOR.tools.htmlEncode(r[0]));
							"undefined" != typeof a.inputStyle && (a.style = a.inputStyle);
				...
				html: function() {
					...
					return function(d, e, f) {
						if (! (3 > arguments.length)) {
							...
							if (q) {
								var r = this.focus;
								this.focus = function() { ("function" == typeof q ? q: r).call(this);
									this.fire("focus")
								};
			...
			CKEDITOR.ui.dialog.textInput.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.labeledElement, {
				...
				setValue: function(a) {
					if (this.bidi) {
						var b = a && a.charAt(0); (b = "‪" == b ? "ltr": "‫" == b ? "rtl": null) && (a = a.slice(1));
			...
			CKEDITOR.dialog.addUIElement("fieldset", {
				build: function(a, b, c) {
					...
					e, f = [], l = [], m = 0; m < d.length && (e = d[m]); m++) {
						var q = [];
						f.push(q);
						l.push(CKEDITOR.dialog._.uiElementBuilders[e.type].build(a, e, q))
					}
	...
	function() {
		...
		function m(a) {
			var b = CKEDITOR.dom.element.createFromHtml(CKEDITOR.addTemplate("dialog", p).output({
		...
		function g(a) {
			function b(c) {
				var r = "rtl" == f.lang.dir,
				n = m.width,
				q = m.height,
				p = n + (c.data.$.screenX - l.x) * (r ? -1 : 1) * (a._.moved ? 1 : 2),
				z = q + (c.data.$.screenY - l.y) * (a._.moved ? 1 : 2),
				u = a._.element.getFirst(),
				u = r && u.getComputedStyle("right"),
				w = a.getPosition();
				w.y + z > k.height && (z = k.height - w.y); (r ? u: w.x) + p > k.width && (p = k.width - (r ? u: w.x));
				if (e == CKEDITOR.DIALOG_RESIZE_WIDTH || e == CKEDITOR.DIALOG_RESIZE_BOTH) n = Math.max(d.minWidth || 0, p - g);
				if (e == CKEDITOR.DIALOG_RESIZE_HEIGHT || e == CKEDITOR.DIALOG_RESIZE_BOTH) q = Math.max(d.minHeight || 0, z - h);
				a.resize(n, q);
				a._.moved || a.layout();
				c.data.preventDefault()
			}
			...
			if (e != CKEDITOR.DIALOG_RESIZE_NONE) {
				...
				n = CKEDITOR.tools.addFunction(function(d) {
					...
					e.$.getElementsByTagName("iframe").length && (r = CKEDITOR.dom.element.createFromHtml('\x3cdiv class\x3d"cke_dialog_resize_cover" style\x3d"height: 100%; position: absolute; width: 100%;"\x3e\x3c/div\x3e'), e.append(r));
		...
		function v(a) {
			...
			g = C[c];
			g ? g.show() : (f = ['\x3cdiv tabIndex\x3d"-1" style\x3d"position: ', CKEDITOR.env.ie6Compat ? "absolute": "fixed", "; z-index: ", f, "; top: 0px; left: 0px; ", CKEDITOR.env.ie6Compat ? "": "background-color: " + e, '" class\x3d"cke_dialog_background_cover"\x3e'], CKEDITOR.env.ie6Compat && (e = "\x3chtml\x3e\x3cbody style\x3d\\'background-color:" + e + ";\\'\x3e\x3c/body\x3e\x3c/html\x3e", f.push('\x3ciframe hidefocus\x3d"true" frameborder\x3d"0" id\x3d"cke_dialog_background_iframe" src\x3d"javascript:'), f.push("void((function(){" + encodeURIComponent("document.open();(" + CKEDITOR.tools.fixDomain + ")();document.write( '" + e + "' );document.close();") + "})())"), f.push('" style\x3d"position:absolute;left:0;top:0;width:100%;height: 100%;filter: progid:DXImageTransform.Microsoft.Alpha(opacity\x3d0)"\x3e\x3c/iframe\x3e')), f.push("\x3c/div\x3e"), g = CKEDITOR.dom.element.createFromHtml(f.join("")), g.setOpacity(void 0 !== d ? d: .5), g.on("keydown", n), g.on("keypress", n), g.on("keyup", n), g.appendTo(CKEDITOR.document.getBody()), C[c] = g);
		...
		CKEDITOR.dialog = function(b, d) {
			...
			function r(c) {
				if (x == CKEDITOR.dialog._.currentTop) {
					...
					g = [37, 38, 39, 40];
					v = y = 0;
					if (9 == d || d == CKEDITOR.SHIFT + 9) l(d == CKEDITOR.SHIFT + 9 ? -1 : 1),
					v = 1;
					else if (d == CKEDITOR.ALT + 121 && !x._.tabBarMode && 1 < x.getPageCount()) x._.tabBarMode = !0,
					x._.tabs[x._.currentTabId][0].focus(),
					x._.currentFocusIndex = -1,
					v = 1;
					else if ( - 1 != CKEDITOR.tools.indexOf(g, d) && x._.tabBarMode) d = -1 != CKEDITOR.tools.indexOf([e ? 39 : 37, 38], d) ? a.call(x) : f.call(x),
					x.selectPage(d),
					x._.tabs[d][0].focus(),
					v = 1;
					else if (13 != d && 32 != d || !x._.tabBarMode) if (13 == d) d = c.data.getTarget(),
					d.is("a", "button", "select", "textarea") || d.is("input") && "button" == d.$.type || ((d = this.getButton("ok")) && CKEDITOR.tools.setTimeout(d.click, 0, d), v = 1),
					y = 1;
			...
			z = CKEDITOR.tools.clone(q),
			w = b.config.dialog_buttonsOrder || "OS",
			A = b.lang.dir,
			t = {},
			v,
			y; ("OS" == w && CKEDITOR.env.mac || "rtl" == w && "ltr" == A || "ltr" == w && "rtl" == A) && z.buttons.reverse();
			p = CKEDITOR.tools.extend(p(b), z);
			...
			if (! ("removeDialogTabs" in b._) && b.config.removeDialogTabs) {
				...
				for (A = 0; A < z.length; A++) if (w = z[A].split(":"), 2 == w.length) {
					var C = w[0];
					t[C] || (t[C] = []);
					t[C].push(w[1])
				}
			...
			if (b._.removeDialogTabs && (t = b._.removeDialogTabs[d])) for (A = 0; A < t.length; A++) p.removeContents(t[A]);
			...
			function(a) {
				B(function(c) {
					if (c.isChanged()) return b.config.dialog_noConfirmCancel || confirm(b.lang.common.confirmCancel) || (a.data.hide = !1),
					!0
				})
			},
			this, null, 0);
			...
			function() {
				D.on("keydown", r, this);
				if (CKEDITOR.env.gecko) D.on("keypress", n, this)
			});
			this.on("hide",
			function() {
				D.removeListener("keydown", r);
				CKEDITOR.env.gecko && D.removeListener("keypress", n);
				B(function(a) {
					e.apply(a)
				})
			});
			this.on("iframeAdded",
			function(a) { (new CKEDITOR.dom.document(a.data.iframe.$.contentWindow.document)).on("keydown", r, this, null, 0)
			});
			this.on("show",
			function() {
				h();
				var a = 1 < x._.pageCount;
				b.config.dialog_startupFocusTab && a ? (x._.tabBarMode = !0, x._.tabs[x._.currentTabId][0].focus(), x._.currentFocusIndex = -1) : this._.hasFocus || (this._.currentFocusIndex = a ? -1 : this._.focusList.length - 1, p.onFocus ? (a = p.onFocus.call(this)) && a.focus() : l(1))
			},
			...
			this);
			k(this);
			g(this); (new CKEDITOR.dom.text(p.title, CKEDITOR.document)).appendTo(this.parts.title);
			for (A = 0; A < p.contents.length; A++)(t = p.contents[A]) && this.addPage(t);
			...
			for (A = 0; A < t.length; A++) this._.buttons[t[A].id] = t[A]
		};
		CKEDITOR.dialog.prototype = {
			...
			show: function() {
				...
				null === CKEDITOR.dialog._.currentTop ? (CKEDITOR.dialog._.currentTop = this, this._.parentDialog = null, v(this._.editor)) : (this._.parentDialog = CKEDITOR.dialog._.currentTop, this._.parentDialog.getElement().getFirst().$.style.zIndex -= Math.floor(this._.editor.config.baseFloatZIndex / 2), CKEDITOR.dialog._.currentTop = this);
				a.on("keydown", D);
				a.on("keyup", F);
				this._.hasFocus = !1;
		...
		r = function(a, b, c) {
			for (var d = 0,
			e; e = a[d]; d++) if (e.id == b || c && e[c] && (e = r(e[c], b, c))) return e;
			return null
		},
		...
		D = function(a) {
			...
			c = a.data.$.altKey,
			d = a.data.$.shiftKey,
			e = String.fromCharCode(a.data.$.keyCode); (b = B[(b ? "CTRL+": "") + (c ? "ALT+": "") + (d ? "SHIFT+": "") + e]) && b.length && (b = b[b.length - 1], b.keydown && b.keydown.call(b.uiElement, b.dialog, b.key), a.data.preventDefault())
		},
		F = function(a) {
			var b = a.data.$.ctrlKey || a.data.$.metaKey,
			c = a.data.$.altKey,
			d = a.data.$.shiftKey,
			e = String.fromCharCode(a.data.$.keyCode); (b = B[(b ? "CTRL+": "") + (c ? "ALT+": "") + (d ? "SHIFT+": "") + e]) && b.length && (b = b[b.length - 1], b.keyup && (b.keyup.call(b.uiElement, b.dialog, b.key), a.data.preventDefault()))
		},
		G = function(a, b, c, d, e) { (B[c] || (B[c] = [])).push({
				uiElement: a,
				dialog: b,
				key: c,
			...
			CKEDITOR.ui.dialog = {
				uiElement: function(a, b, c, d, e, f, g) {
					if (! (4 > arguments.length)) {
						...
						l = (e && e.call ? e(b) : e) || {},
						m = (f && f.call ? f(b) : f) || {},
						r = (g && g.call ? g.call(this, a, b) : g) || "",
						n = this.domId = m.id || CKEDITOR.tools.getNextId() + "_uiElement";
						b.requiredContent && !a.getParentEditor().filter.check(b.requiredContent) && (l.display = "none", this.notAllowed = !0);
						m.id = n;
						var q = {};
						b.type && (q["cke_dialog_ui_" + b.type] = 1);
						b.className && (q[b.className] = 1);
						b.disabled && (q.cke_disabled = 1);
						for (var p = m["class"] && m["class"].split ? m["class"].split(" ") : [], n = 0; n < p.length; n++) p[n] && (q[p[n]] = 1);
						p = [];
						for (n in q) p.push(n);
						m["class"] = p.join(" ");
						b.title && (m.title = b.title);
						q = (b.style || "").split(";");
						b.align && (p = b.align, l["margin-left"] = "left" == p ? 0 : "auto", l["margin-right"] = "right" == p ? 0 : "auto");
						for (n in l) q.push(n + ":" + l[n]);
						b.hidden && q.push("display:none");
						for (n = q.length - 1; 0 <= n; n--)"" === q[n] && q.splice(n, 1);
						0 < q.length && (m.style = (m.style ? m.style + "; ": "") + q.join("; "));
						for (n in m) k.push(n + '\x3d"' + CKEDITOR.tools.htmlEncode(m[n]) + '" ');
						k.push("\x3e", r, "\x3c/", h, "\x3e");
					...
					if (! (4 > arguments.length)) {
						...
						function() {
							...
							for (k = 0; k < c.length; k++) {
								...
								g ? g[k] && d.push("width:" + w(g[k])) : d.push("width:" + Math.floor(100 / c.length) + "%");
								h && d.push("height:" + w(h));
								e && void 0 !== e.padding && d.push("padding:" + w(e.padding));
					...
					if (! (3 > arguments.length)) {
						...
						function() {
							...
							for (var d = 0; d < c.length; d++) {
								...
								g && k.push("width:" + w(g || "100%"));
								h ? k.push("height:" + w(h[d])) : e && e.expand && k.push("height:" + Math.floor(100 / c.length) + "%");
			...
			init: function(a) {
				...
				a.addCommand("a11yHelp", {
					exec: function() {
						...
						CKEDITOR.scriptLoader.load(CKEDITOR.getUrl(f.path + "dialogs/lang/" + b + ".js"),
						function() {
		...
		init: function(a) {
			...
			b = function(b, d, e, k) {
				if (k) {
					...
					function(b) { ! a.readOnly && a.getCommand(e).setState(b)
					});
				...
				if (e) {
					...
					if (b == CKEDITOR.TRISTATE_OFF) {
						...
						for (; 0 < e.length;) if (b = e.shift(), "blockquote" == b.getName()) {
							for (d = new CKEDITOR.dom.documentFragment(a.document); b.getFirst();) d.append(b.getFirst().remove()),
							l.push(d.getLast());
							d.replace(b)
						} else l.push(b);
						e = a.document.createElement("blockquote");
						for (e.insertBefore(l[0]); 0 < l.length;) b = l.shift(),
						e.append(b)
						...
						b = [];
						l = [];
						for (h = {}; 0 < d.length;) k = d.shift(),
						e = k.getParent(),
						k.getPrevious() ? k.getNext() ? (k.breakParent(k.getParent()), l.push(k.getNext())) : k.remove().insertAfter(e) : k.remove().insertBefore(e),
						e.getCustomData("blockquote_processed") || (l.push(e), CKEDITOR.dom.element.setMarker(h, e, "blockquote_processed", !0)),
						b.push(k);
						CKEDITOR.dom.element.clearAllMarkers(h);
						for (d = l.length - 1; 0 <= d; d--) {
							...
							a: {
								h = e;
								for (var k = 0,
								g = h.getChildCount(), n = void 0; k < g && (n = h.getChild(k)); k++) if (n.type == CKEDITOR.NODE_ELEMENT && n.isBlockBoundary()) {
									h = !1;
									break a
								}
								h = !0
						...
						if (a.config.enterMode == CKEDITOR.ENTER_BR) for (e = !0; b.length;) if (k = b.shift(), "div" == k.getName()) {
							d = new CKEDITOR.dom.documentFragment(a.document); ! e || !k.getPrevious() || k.getPrevious().type == CKEDITOR.NODE_ELEMENT && k.getPrevious().isBlockBoundary() || d.append(a.document.createElement("br"));
							for (e = k.getNext() && !(k.getNext().type == CKEDITOR.NODE_ELEMENT && k.getNext().isBlockBoundary()); k.getFirst();) k.getFirst().remove().appendTo(d);
	...
	function() {
		function a(a, b, c) {
			...
			if (CKEDITOR.env.gecko && "drop" == b.method && a.toolbox) a.once("afterPaste",
			function() {
				a.toolbox.focus()
			});
		...
		function f(b) {
			function c() {
				...
				function(a) {
					"beforepaste" == B.mainPasteEvent && D || y(a)
				});
				"beforepaste" == B.mainPasteEvent && (a.on("paste",
				function(a) {
					F || (f(), a.data.preventDefault(), y(a), k("paste") || b.openDialog("paste"))
				}), a.on("contextmenu", h, null, null, 0), a.on("beforepaste",
				function(a) { ! a.data || a.data.$.ctrlKey || a.data.$.shiftKey || h()
				},
				null, null, 0));
				a.on("beforecut",
				function() { ! D && l(b)
				...
				var e;
				a.attachListener(CKEDITOR.env.ie ? a: b.document.getDocumentElement(), "mouseup",
				function() {
				...
				return {
					...
					exec: function(b, c) {
						...
						"string" == typeof c ? e({
							dataValue: c,
							method: "paste",
							dataTransfer: B.initPasteDataTransfer()
						},
			...
			function m(a, c) {
				...
				if (!d.getById("cke_pastebin")) {
					...
					var r = new CKEDITOR.dom.element(!CKEDITOR.env.webkit && !e.is("body") || CKEDITOR.env.ie ? "div": "body", d);
					r.setAttributes({
						id: "cke_pastebin",
						"data-cke-temp": "1"
					});
					var n = 0,
					d = d.getWindow();
					CKEDITOR.env.webkit ? (e.append(r), r.addClass("cke_editable"), e.is("body") || (n = "static" != e.getComputedStyle("position") ? e: CKEDITOR.dom.element.get(e.$.offsetParent), n = n.getDocumentPosition().y)) : e.getAscendant(CKEDITOR.env.ie ? "body": "html", 1).append(r);
					r.setStyles({
						position: "absolute",
					...
					CKEDITOR.env.safari && r.setStyles(CKEDITOR.tools.cssVendorPrefix("user-select", "text")); (n = r.getParent().isReadOnly()) ? (r.setOpacity(0), r.setAttribute("contenteditable", !0)) : r.setStyle("ltr" == b.config.contentsLangDirection ? "left": "right", "-10000px");
					b.on("selectionChange", f, null, null, 0);
					if (CKEDITOR.env.webkit || CKEDITOR.env.gecko) h = e.once("blur", f, null, null, -100);
					n && r.focus();
					n = new CKEDITOR.dom.range(r);
					n.selectNodeContents(r);
					var q = n.select();
					CKEDITOR.env.ie && (h = e.once("blur",
					function() {
						b.lockSelection(q)
					}));
					...
					setTimeout(function() {
						...
						r.remove();
						var a;
						CKEDITOR.env.webkit && (a = r.getFirst()) && a.is && a.hasClass("Apple-style-span") && (r = a);
			...
			function A() {
				...
				a.lock();
				if (b.editable().fire(B.mainPasteEvent) && !k("paste")) return a.unlock(),
				!1;
				a.unlock();
				return ! 0
			}
			function u(a) {
				...
				case CKEDITOR.CTRL + 86 : case CKEDITOR.SHIFT + 45 : a = b.editable();
					f();
					"paste" == B.mainPasteEvent && a.fire("beforepaste");
					break;
				case CKEDITOR.CTRL + 88 : case CKEDITOR.SHIFT + 46 : b.fire("saveSnapshot"),
					setTimeout(function() {
						b.fire("saveSnapshot")
					},
					50)
			...
			b.getClipboardData = function(a, c) {
				...
				null, null, 1E3); ! 1 === A() && (b.removeListener("paste", d), h && b.fire("pasteDialog", f) ? (b.on("pasteDialogCommit", e), b.on("dialogHide",
				function(a) {
					a.removeListener();
		...
		function c(a, b) {
			...
			CKEDITOR.env.gecko && a.enterMode != CKEDITOR.ENTER_BR && (CKEDITOR.env.gecko && (b = b.replace(/^<br><br>$/, "\x3cbr\x3e")), -1 < b.indexOf("\x3cbr\x3e\x3cbr\x3e") && (b = "\x3cp\x3e" + b.replace(/(<br>){2,}/g,
			function(a) {
				return c(a.length / 4)
			}) + "\x3c/p\x3e"));
			return l(a, b)
		...
		function h(b) {
			...
			function() {
				...
				function f(a, c, d) {
					...
					c && (e.dragRange = c);
					d && (e.dropRange = d); ! 1 === b.fire(a.name, e) && a.data.preventDefault()
				}
				function h(a) {
			...
			init: function(a) {
				...
				if (CKEDITOR.env.gecko) {
					...
					function(b) {
						var c = b.data,
						d = c.dataTransfer;
						if (!c.dataValue && "paste" == c.method && d && 1 == d.getFilesCount() && w != d.id && (d = d.getFile(0), -1 != CKEDITOR.tools.indexOf(l, d.type))) {
		...
		CKEDITOR.plugins.clipboard = {
			...
			fixSplitNodesAfterDrop: function(a, b, c, d) {
				function e(a, c, d) {
					...
					if (f.equals(c) && d != c.getChildCount()) return a = b.startContainer.getChild(b.startOffset - 1),
					c = b.startContainer.getChild(b.startOffset),
					a && a.type == CKEDITOR.NODE_TEXT && c && c.type == CKEDITOR.NODE_TEXT && (d = a.getLength(), a.setText(a.getText() + c.getText()), c.remove(), b.setStart(a, d), b.collapse(!0)),
					!0
				}
			...
			internalDrop: function(b, c, d, e) {
				...
				CKEDITOR.env.ie && 10 > CKEDITOR.env.version && this.fixSplitNodesAfterDrop(b, c, f.dragStartContainerChildCount, f.dragEndContainerChildCount); (l = this.isDropRangeAffectedByDragRange(b, c)) || (k = b.createBookmark(!1));
			...
			getRangeAtDropPosition: function(a, b) {
				...
				if (document.caretRangeFromPoint) c = b.document.$.caretRangeFromPoint(d, e),
					...
					if (document.body.createTextRange) {
						...
						try {
							...
							if (k) {
								...
								B = new CKEDITOR.dom.element(x),
								D;
								if (B.equals(b.editable()) || "html" == B.getName()) return f && f.startContainer && !f.startContainer.equals(b.editable()) ? f: null;
								D = B.getClientRect();
								d < D.left ? h.setStartAt(B, CKEDITOR.POSITION_AFTER_START) : h.setStartAt(B, CKEDITOR.POSITION_BEFORE_END);
								h.collapse(!0)
							}
						} catch(F) {
							return null
	...
	function() {
		...
		CKEDITOR.ui.panel.block = CKEDITOR.tools.createClass({
			...
			_: {
				markItem: function(a) { - 1 != a && (a = this.element.getElementsByTag("a").getItem(this._.focusIndex = a), CKEDITOR.env.webkit && a.getDocument().getWindow().focus(), a.focus(), this.onMark && this.onMark(a))
				}
			},
			proto: {
				show: function() {
				...
				onKeyDown: function(a, b) {
					...
					switch (f) {
					case "next":
						for (var l = this._.focusIndex,
						f = this.element.getElementsByTag("a"), d; d = f.getItem(++l);) if (d.getAttribute("_cke_focus") && d.$.offsetWidth) {
							this._.focusIndex = l;
							d.focus();
							break
						}
					...
					case "click":
					case "mouseup":
						return l = this._.focusIndex,
						(d = 0 <= l && this.element.getElementsByTag("a").getItem(l)) && (d.$[f] ? d.$[f]() : d.$["on" + f]()),
						!1
					}
					return ! 0
				}
	...
	function() {
		...
		CKEDITOR.ui.floatPanel = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				showBlock: function(a, c, e, f, l, d) {
					...
					w = c.getDocumentPosition(v),
					v = t ? t.getDocumentPosition(v) : {
						x: 0,
						y: 0
					},
					p = "rtl" == this._.dir,
					q = w.x + (f || 0) - v.x,
					r = w.y + (l || 0) - v.y; ! p || 1 != e && 4 != e ? p || 2 != e && 3 != e || (q += c.$.offsetWidth - 1) : q += c.$.offsetWidth;
					if (3 == e || 4 == e) r += c.$.offsetHeight - 1;
					...
					this._.blurSet || (CKEDITOR.event.useCapture = !0, g.on("blur",
					function(a) {
						function b() {
					...
					this), CKEDITOR.env.iOS && (g.on("touchstart",
					function() {
					...
					CKEDITOR.tools.setTimeout(function() {
						var a = CKEDITOR.tools.bind(function() {
							...
							f = p ? a.right: b.width - a.left,
							g = p ? b.width - a.right: a.left;
							p ? f < c && (q = g > c ? q + c: b.width > c ? q - a.left: q - a.right + b.width) : f < c && (q = g > c ? q - c: b.width > c ? q - a.right + b.width: q - a.left);
							c = a.top;
							...
							var a = n.getFirst(),
							l; (l = a.getCustomData("activePanel")) && l.onHide && l.onHide.call(this, 1);
				...
				showAsChild: function(a, c, e, f, l, d) {
					if (this._.activeChild != a || a._.panel._.offsetParentId != e.getId()) this.hideChild(),
		...
		requires: "floatpanel",
		beforeInit: function(a) {
			for (var f = a.config.menu_groups.split(","), b = a._.menuGroups = {},
			c = a._.menuItems = {},
			e = 0; e < f.length; e++) b[f[e]] = e + 1;
			...
			_: {
				onShow: function() {
					...
					for (var f = 0; f < e.length; f++) {
						var g = e[f](b, a, c);
						if (g) for (var n in g) {
							var v = this.editor.getMenuItem(n); ! v || v.command && !this.editor.getCommand(v.command).state || (v.state = g[n], this.add(v))
						}
					}
				},
				onClick: function(a) {
			...
			proto: {
				...
				show: function(b, c, d, e) {
					...
					if (!n) {
						...
						this);
						v = n.addBlock(this.id, this._.panelDefinition.block);
						v.autoSize = !0;
						var t = v.keys;
						t[40] = "next";
						t[9] = "next";
					...
					a(f);
					for (var t = g.elementPath(), t = ['\x3cdiv class\x3d"cke_menu' + (t && t.direction() != g.lang.dir ? " cke_mixed_dir_content": "") + '" role\x3d"presentation"\x3e'], w = f.length, p = w && f[0].group, q = 0; q < w; q++) {
						var r = f[q];
						p != r.group && (t.push('\x3cdiv class\x3d"cke_menuseparator" role\x3d"separator"\x3e\x3c/div\x3e'), p = r.group);
						r.render(this, q, t)
					}
					t.push("\x3c/div\x3e");
					v.setHtml(t.join(""));
		...
		CKEDITOR.menuItem = CKEDITOR.tools.createClass({
			...
			proto: {
				render: function(a, f, d) {
					...
					v,
					t,
					w = k == CKEDITOR.TRISTATE_ON ? "on": k == CKEDITOR.TRISTATE_DISABLED ? "disabled": "off";
					this.role in {
						menuitemcheckbox: 1,
						menuitemradio: 1
					...
					this.command && (v = n.getCommand(this.command), (v = n.getCommandKeystroke(v)) && (t = CKEDITOR.tools.keystrokeToString(n.lang.common.keyboard, v)));
					a = {
						id: h,
						name: this.name,
						iconName: r,
						...
						shortcutHtml: t ? e.output({
							shortcut: t.display
						}) : "",
						arrowHtml: p ? c.output({
							label: q
						}) : "",
	...
	function() {
		function a(a, b) {
			...
			t = CKEDITOR.tools.addFunction(function(b, c) {
				...
				case f ? 39 : 37 : case 9:
					return (f = CKEDITOR.document.getById(d + (b + 1))) || (f = CKEDITOR.document.getById(d + "0")),
					f.focus(),
					!1;
				case f ? 37 : 39 : case CKEDITOR.SHIFT + 9 : return (f = CKEDITOR.document.getById(d + (b - 1))) || (f = CKEDITOR.document.getById(d + (g.list.length - 1))),
					f.focus(),
					!1;
				case 27:
					return a.focus(),
			...
			function() {
				for (var b = [], d = g.list = [], f = [], l = g.filters, m = !0, A = a.elementPath().elements, u, y = A.length; y--;) {
					var C = A[y],
					x = 0;
					u = C.data("cke-display-name") ? C.data("cke-display-name") : C.data("cke-real-element-type") ? C.data("cke-real-element-type") : C.getName(); (m = C.hasAttribute("contenteditable") ? "true" == C.getAttribute("contenteditable") : m) || C.hasAttribute("contenteditable") || (x = 1);
					...
					x || (d.unshift(C), f.unshift(u))
				}
				d = d.length;
				for (l = 0; l < d; l++) u = f[l],
				m = a.lang.elementspath.eleTitle.replace(/%1/, u),
				u = c.output({
					id: n + l,
					label: m,
					text: u,
		...
		CKEDITOR.plugins.add("indent", {
			init: function(b) {
				...
				function(a) {
					...
					e.setStartBefore(d);
					e.setEndAfter(d);
					for (var f = new CKEDITOR.dom.walker(e), k; k = f.next();) if (k.type == CKEDITOR.NODE_ELEMENT) if (!k.equals(d) && k.getDirection()) e.setStartAfter(k),
					f = new CKEDITOR.dom.walker(e);
					else {
						var g = b.config.indentClasses;
						if (g) for (var n = "ltr" == a.data.dir ? ["_rtl", ""] : ["", "_rtl"], v = 0; v < g.length; v++) k.hasClass(g[v] + n[0]) && (k.removeClass(g[v] + n[0]), k.addClass(g[v] + n[1]));
	...
	function() {
		function a(a) {
			function c(f) {
				for (var h = t.startContainer,
				m = t.endContainer; h && !h.getParent().equals(f);) h = h.getParent();
				for (; m && !m.getParent().equals(f);) m = m.getParent();
				if (!h || !m) return ! 1;
				for (var p = h,
				h = [], w = !1; ! w;) p.equals(m) && (w = !0),
				h.push(p),
				p = p.getNext();
				if (1 > h.length) return ! 1;
				p = f.getParents(!0);
				for (m = 0; m < p.length; m++) if (p[m].getName && l[p[m].getName()]) {
					f = p[m];
					break
				}
				for (var p = e.isIndent ? 1 : -1, m = h[0], h = h[h.length - 1], w = CKEDITOR.plugins.list.listToArray(f, g), y = w[h.getCustomData("listarray_index")].indent, m = m.getCustomData("listarray_index"); m <= h.getCustomData("listarray_index"); m++) if (w[m].indent += p, 0 < p) {
					var v = w[m].parent;
					w[m].parent = new CKEDITOR.dom.element(v.getName(), v.getDocument())
				}
				for (m = h.getCustomData("listarray_index") + 1; m < w.length && w[m].indent > y; m++) w[m].indent += p;
				h = CKEDITOR.plugins.list.arrayToList(w, g, null, a.config.enterMode, f.getDirection());
				if (!e.isIndent) {
					var x;
					if ((x = f.getParent()) && x.is("li")) for (var p = h.listNode.getChildren(), B = [], D, m = p.count() - 1; 0 <= m; m--)(D = p.getItem(m)) && D.is && D.is("li") && B.push(D)
				}
				h && h.listNode.replace(f);
				if (B && B.length) for (m = 0; m < B.length; m++) {
					for (D = f = B[m]; (D = D.getNext()) && D.is && D.getName() in l;) CKEDITOR.env.needsNbspFiller && !f.getFirst(b) && f.append(t.document.createText(" ")),
					f.append(D);
					f.insertAfter(x)
				}
			...
			g = this.database,
			l = this.context,
			m = a.getSelection(), m = (m && m.getRanges()).createIterator(), t; t = m.getNextRange();) {
				for (var w = t.getCommonAncestor(); w && (w.type != CKEDITOR.NODE_ELEMENT || !l[w.getName()]);) {
					if (a.editable().equals(w)) {
						w = !1;
						break
					}
					w = w.getParent()
				}
				w || (w = t.startPath().contains(l)) && t.setEndAt(w, CKEDITOR.POSITION_BEFORE_END);
				if (!w) {
					var p = t.getEnclosedNode();
					p && p.type == CKEDITOR.NODE_ELEMENT && p.getName() in l && (t.setStartAt(p, CKEDITOR.POSITION_AFTER_START), t.setEndAt(p, CKEDITOR.POSITION_BEFORE_END), w = p)
				}
				w && t.startContainer.type == CKEDITOR.NODE_ELEMENT && t.startContainer.getName() in l && (p = new CKEDITOR.dom.walker(t), p.evaluator = f, t.startContainer = p.next());
				w && t.endContainer.type == CKEDITOR.NODE_ELEMENT && t.endContainer.getName() in l && (p = new CKEDITOR.dom.walker(t), p.evaluator = f, t.endContainer = p.previous());
				if (w) return c(w)
			}
			return 0
		}
	...
	function() {
		...
		function b(a, b, c, d) {
			for (var e, f; e = a[d ? "getLast": "getFirst"](t);)(f = e.getDirection(1)) !== b.getDirection(1) && e.setAttribute("dir", f),
			e.remove(),
		...
		function m(a, d, e) {
			...
			t = new CKEDITOR.dom.elementPath(e.startContainer),
			v = t.contains(CKEDITOR.dtd.$listItem),
			t = t.contains(CKEDITOR.dtd.$list);
			m ? (m = m.getBogus()) && m.remove() : t && (m = t.getPrevious(g)) && n(m) && m.remove(); (m = f.getLast()) && m.type == CKEDITOR.NODE_ELEMENT && m.is("br") && m.remove(); (m = d.startContainer.getChild(d.startOffset)) ? f.insertBefore(m) : d.startContainer.append(f);
			v && (f = l(v)) && (k.contains(v) ? (b(f, v.getParent(), v), f.remove()) : k.append(f));
		...
		CKEDITOR.plugins.list = {
			listToArray: function(a, b, c, e, f) {
				...
				h = a.getChildCount(); g < h; g++) {
					...
					if ("li" == k.$.nodeName.toLowerCase()) {
						...
						c.push(l);
						for (var m = 0,
						n = k.getChildCount(), t; m < n; m++) t = k.getChild(m),
						t.type == CKEDITOR.NODE_ELEMENT && d[t.getName()] ? CKEDITOR.plugins.list.listToArray(t, b, c, e + 1, l.grandparent) : l.contents.push(t)
					}
				}
				return c
			},
			arrayToList: function(a, b, c, e, f) {
				...
				for (var h, l = a[c].parent.getDocument(), m = new CKEDITOR.dom.documentFragment(l), n = null, t = c, v = Math.max(a[c].indent, 0), D = null, F, G, I = e == CKEDITOR.ENTER_P ? "p": "div";;) {
					var H = a[t];
					h = H.grandparent;
					F = H.element.getDirection(1);
					if (H.indent == v) {
						n && a[t].parent.getName() == n.getName() || (n = a[t].parent.clone(!1, 1), f && n.setAttribute("dir", f), m.append(n));
						D = n.append(H.element.clone(0, 1));
						F != n.getDirection(1) && D.setAttribute("dir", F);
						for (h = 0; h < H.contents.length; h++) D.append(H.contents[h].clone(1, 1));
						t++
					} else if (H.indent == Math.max(v, 0) + 1) H = a[t - 1].element.getDirection(1),
					t = CKEDITOR.plugins.list.arrayToList(a, null, t, e, H != F ? F: null),
					!D.getChildCount() && CKEDITOR.env.needsNbspFiller && 7 >= l.$.documentMode && D.append(l.createText(" ")),
					D.append(t.listNode),
					t = t.nextIndex;
					else if ( - 1 == H.indent && !c && h) {
						d[h.getName()] ? (D = H.element.clone(!1, !0), F != h.getDirection(1) && D.setAttribute("dir", F)) : D = new CKEDITOR.dom.documentFragment(l);
						var n = h.getDirection(1) != F,
						K = H.element,
						J = K.getAttribute("class"),
						E = K.getAttribute("style"),
						S = D.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT && (e != CKEDITOR.ENTER_BR || n || E || J),
						O,
						T = H.contents.length,
						M;
						for (h = 0; h < T; h++) if (O = H.contents[h], k(O) && 1 < T) S ? M = O.clone(1, 1) : D.append(O.clone(1, 1));
							...
							G = null;
							M && (D.append(M), M = null);
							D.append(O.clone(1, 1))
						} else S ? (G || (G = l.createElement(I), D.append(G), n && G.setAttribute("dir", F)), E && G.setAttribute("style", E), J && G.setAttribute("class", J), M && (G.append(M), M = null), G.append(O.clone(1, 1))) : D.append(O.clone(1, 1));
						M && ((G || D).append(M), M = null);
						D.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT && t != a.length - 1 && (CKEDITOR.env.needsBrFiller && (F = D.getLast()) && F.type == CKEDITOR.NODE_ELEMENT && F.is("br") && F.remove(), (F = D.getLast(g)) && F.type == CKEDITOR.NODE_ELEMENT && F.is(CKEDITOR.dtd.$block) || D.append(l.createElement("br")));
					...
					G = null;
					if (a.length <= t || Math.max(a[t].indent, 0) < v) break
				}
		...
		f.prototype = {
			exec: function(b) {
				...
				if (this.state == CKEDITOR.TRISTATE_OFF) {
					var k = b.editable();
					if (k.getFirst(g)) {
						var l = 1 == h.length && h[0]; (e = l && l.getEnclosedNode()) && e.is && this.type == e.getName() && this.setState(CKEDITOR.TRISTATE_ON)
					} else e.enterMode == CKEDITOR.ENTER_BR ? k.appendBogus() : h[0].fixBlock(1, e.enterMode == CKEDITOR.ENTER_P ? "p": "div"),
					f.selectRanges(h)
				}
				for (var e = f.createBookmarks(!0), k = [], m = {},
				h = h.createIterator(), n = 0; (l = h.getNextRange()) && ++n;) {
					var t = l.getBoundaryNodes(),
					x = t.startNode,
					B = t.endNode;
					x.type == CKEDITOR.NODE_ELEMENT && "td" == x.getName() && l.setStartAt(t.startNode, CKEDITOR.POSITION_AFTER_START);
					B.type == CKEDITOR.NODE_ELEMENT && "td" == B.getName() && l.setEndAt(t.endNode, CKEDITOR.POSITION_BEFORE_END);
					l = l.createIterator();
					for (l.forceBrBreak = this.state == CKEDITOR.TRISTATE_OFF; t = l.getNextParagraph();) if (!t.getCustomData("list_block")) {
						CKEDITOR.dom.element.setMarker(m, t, "list_block", 1);
						for (var D = b.elementPath(t), x = D.elements, B = 0, D = D.blockLimit, F, G = x.length - 1; 0 <= G && (F = x[G]); G--) if (d[F.getName()] && D.contains(F)) {
							D.removeCustomData("list_group_object_" + n); (x = F.getCustomData("list_group_object")) ? x.contents.push(t) : (x = {
								root: F,
								contents: [t]
							},
						...
						B || (B = D, B.getCustomData("list_group_object_" + n) ? B.getCustomData("list_group_object_" + n).contents.push(t) : (x = {
							root: B,
							contents: [t]
						},
						CKEDITOR.dom.element.setMarker(m, B, "list_group_object_" + n, x), k.push(x)))
					}
				}
				for (F = []; 0 < k.length;) if (x = k.shift(), this.state == CKEDITOR.TRISTATE_OFF) if (d[x.root.getName()]) {
					h = b;
					n = x;
					x = m;
					l = F;
					B = CKEDITOR.plugins.list.listToArray(n.root, x);
					D = [];
					for (t = 0; t < n.contents.length; t++) G = n.contents[t],
					(G = G.getAscendant("li", !0)) && !G.getCustomData("list_item_processed") && (D.push(G), CKEDITOR.dom.element.setMarker(x, G, "list_item_processed", !0));
					for (var G = n.root.getDocument(), I = void 0, H = void 0, t = 0; t < D.length; t++) {
						var K = D[t].getCustomData("listarray_index"),
						I = B[K].parent;
					...
					x = CKEDITOR.plugins.list.arrayToList(B, x, null, h.config.enterMode);
					B = void 0;
					D = x.listNode.getChildCount();
					for (t = 0; t < D && (B = x.listNode.getChild(t)); t++) B.getName() == this.type && l.push(B);
					...
					n = [];
					1 == D.length && D[0].equals(l.root) && (x = h.createElement("div"), D[0].moveChildren && D[0].moveChildren(x), D[0].append(x), D[0] = x);
					l = l.contents[0].getParent();
					for (G = 0; G < D.length; G++) l = l.getCommonAncestor(D[G].getParent());
					I = B.config.useComputedState;
					B = x = void 0;
					I = void 0 === I || I;
					for (G = 0; G < D.length; G++) for (H = D[G]; K = H.getParent();) {
						if (K.equals(l)) {
							n.push(H); ! B && H.getDirection() && (B = 1);
							H = H.getDirection(I);
					...
					if (! (1 > n.length)) {
						...
						t.push(G);
						for (I = t = void 0; n.length;) t = n.shift(),
						I = h.createElement("li"),
						H = t,
						H.is("pre") || v.test(H.getName()) || "false" == H.getAttribute("contenteditable") ? t.appendTo(I) : (t.copyAttributes(I), x && t.getDirection() && (I.removeStyle("direction"), I.removeAttribute("dir")), t.moveChildren(I), t.remove()),
						I.appendTo(G);
						x && B && G.setAttribute("dir", x);
						D ? G.insertBefore(D) : G.appendTo(l)
					}
				...
				function(b) {
					...
					if ("wysiwyg" == a.mode && c in {
						...
						if (h && h.collapsed) {
							...
							c = h.clone();
							if (t) {
								var x; (x = k.contains(d)) && h.checkBoundaryOfElement(x, CKEDITOR.START) && (x = x.getParent()) && x.is("li") && (x = l(x)) ? (f = x, x = x.getPrevious(g), c.moveToPosition(x && n(x) ? x: f, CKEDITOR.POSITION_BEFORE_START)) : (C.range.setStartAt(v, CKEDITOR.POSITION_AFTER_START), C.range.setEnd(h.startContainer, h.startOffset), (x = C.previous()) && x.type == CKEDITOR.NODE_ELEMENT && (x.getName() in d || x.is("li")) && (x.is("li") || (C.range.selectNodeContents(x), C.reset(), C.evaluator = e, x = C.previous()), f = x, c.moveToElementEditEnd(f), c.moveToPosition(c.endPath().block, CKEDITOR.POSITION_BEFORE_END)));
								if (f) m(a, c, h),
								b.cancel();
								else {
									var B = k.contains(d);
									B && h.checkBoundaryOfElement(B, CKEDITOR.START) && (f = B.getFirst(g), h.checkBoundaryOfElement(f, CKEDITOR.START) && (x = B.getPrevious(g), l(f) ? x && (h.moveToElementEditEnd(x), h.select()) : a.execCommand("outdent"), b.cancel()))
								}
							} else if (f = k.contains("li")) {
								if (C.range.setEndAt(v, CKEDITOR.POSITION_BEFORE_END), t = (v = f.getLast(g)) && e(v) ? v: f, k = 0, (x = C.next()) && x.type == CKEDITOR.NODE_ELEMENT && x.getName() in d && x.equals(v) ? (k = 1, x = C.next()) : h.checkBoundaryOfElement(t, CKEDITOR.END) && (k = 2), k && x) {
									h = h.clone();
									h.moveToElementEditStart(x);
									if (1 == k && (c.optimize(), !c.startContainer.equals(f))) {
										for (f = c.startContainer; f.is(CKEDITOR.dtd.$inline);) B = f,
										f = f.getParent();
							...
							} else C.range.setEndAt(v, CKEDITOR.POSITION_BEFORE_END),
							(x = C.next()) && x.type == CKEDITOR.NODE_ELEMENT && x.is(d) && (x = x.getFirst(g), k.block && h.checkStartOfBlock() && h.checkEndOfBlock() ? (k.block.remove(), h.moveToElementEditStart(x), h.select()) : l(x) ? (h.moveToElementEditStart(x), h.select()) : (h = h.clone(), h.moveToElementEditStart(x), m(a, c, h)), b.cancel());
							setTimeout(function() {
								a.selectionChange(1)
							})
						}
	...
	function() {
		...
		CKEDITOR.plugins.enterkey = {
			enterBlock: function(a, d, m, v) {
				if (m = m || f(a)) {
					m = b(m);
					var t = m.document,
					w = m.checkStartOfBlock(),
					p = m.checkEndOfBlock(),
					...
					A;
					if (w && p) {
						if (r && (r.is("li") || r.getParent().is("li"))) {
							r.is("li") || (r = r.getParent());
							m = r.getParent();
							A = m.getParent();
							v = !r.hasPrevious();
							var u = !r.hasNext(),
							...
							y = z.createBookmarks(),
							w = r.getDirection(1),
							p = r.getAttribute("class"),
							C = r.getAttribute("style"),
							x = A.getDirection(1) != w;
							a = a.enterMode != CKEDITOR.ENTER_BR || x || C || p;
							if (A.is("li")) v || u ? (v && u && m.remove(), r[u ? "insertAfter": "insertBefore"](A)) : r.breakParent(A);
							else {
								if (a) if (q.block.is("li") ? (A = t.createElement(d == CKEDITOR.ENTER_P ? "p": "div"), x && A.setAttribute("dir", w), C && A.setAttribute("style", C), p && A.setAttribute("class", p), r.moveChildren(A)) : A = q.block, v || u) A[v ? "insertBefore": "insertAfter"](m);
								else r.breakParent(m),
								A.insertAfter(m);
								else if (r.appendBogus(!0), v || u) for (; t = r[v ? "getFirst": "getLast"]();) t[v ? "insertBefore": "insertAfter"](m);
								else for (r.breakParent(m); t = r.getLast();) t.insertAfter(m);
								r.remove()
							}
							z.selectBookmarks(y);
							return
						...
						if (r && r.getParent().is("blockquote")) {
							...
							m.moveToElementEditStart(r);
							m.select();
							return
						}
					} else if (r && r.is("pre") && !p) {
						l(a, d, m, v);
						return
					}
					if (w = m.splitBlock(z)) {
						...
						q = w.wasStartOfBlock;
						a = w.wasEndOfBlock;
						r ? (y = r.getParent(), y.is("li") && (r.breakParent(y), r.move(r.getNext(), 1))) : d && (y = d.getParent()) && y.is("li") && (d.breakParent(y), y = d.getNext(), m.moveToElementEditStart(y), d.move(d.getPrevious()));
						if (q || a) {
							if (d) {
								if (d.is("li") || !h.test(d.getName()) && !d.is("pre")) A = d.clone()
							} else r && (A = r.clone());
							A ? v && !A.is("li") && A.renameNode(z) : y && y.is("li") ? A = y: (A = t.createElement(z), d && (u = d.getDirection()) && A.setAttribute("dir", u));
							if (t = w.elementPath) for (v = 0, z = t.elements.length; v < z; v++) {
								y = t.elements[v];
								if (y.equals(t.block) || y.equals(t.blockLimit)) break;
								CKEDITOR.dtd.$removeEmpty[y.getName()] && (y = y.clone(), A.moveChildren(y), A.append(y))
							}
							A.appendBogus();
							A.getParent() || m.insertNode(A);
							A.is("li") && A.removeAttribute("value"); ! CKEDITOR.env.ie || !q || a && d.getChildCount() || (m.moveToElementEditStart(a ? d: A), m.select());
							m.moveToElementEditStart(q && !a ? r: A)
						} else r.is("li") && (A = m.clone(), A.selectNodeContents(r), A = new CKEDITOR.dom.walker(A), A.evaluator = function(a) {
							return ! (e(a) || c(a) || a.type == CKEDITOR.NODE_ELEMENT && a.getName() in CKEDITOR.dtd.$inline && !(a.getName() in CKEDITOR.dtd.$empty))
						},
						(y = A.next()) && y.type == CKEDITOR.NODE_ELEMENT && y.is("ul", "ol") && (CKEDITOR.env.needsBrFiller ? t.createElement("br") : t.createText(" ")).insertBefore(y)),
						r && m.moveToElementEditStart(r);
						m.select();
						m.scrollIntoView()
					}
				}
			...
			enterBr: function(a, b, c, e) {
				if (c = c || f(a)) {
					...
					q = p.block,
					r = q && p.block.getName();
					e || "li" != r ? (!e && m && h.test(r) ? (m = q.getDirection()) ? (l = l.createElement("div"), l.setAttribute("dir", m), l.insertAfter(q), c.setStart(l, 0)) : (l.createElement("br").insertAfter(q), CKEDITOR.env.gecko && l.createText("").insertAfter(q), c.setStartAt(q.getNext(), CKEDITOR.env.ie ? CKEDITOR.POSITION_BEFORE_START: CKEDITOR.POSITION_AFTER_START)) : (a = "pre" == r && CKEDITOR.env.ie && 8 > CKEDITOR.env.version ? l.createText("\r") : l.createElement("br"), c.deleteContents(), c.insertNode(a), CKEDITOR.env.needsBrFiller ? (l.createText("﻿").insertAfter(a), m && (q || p.blockLimit).appendBogus(), a.getNext().$.nodeValue = "", c.setStartAt(a.getNext(), CKEDITOR.POSITION_AFTER_START)) : c.setStartAt(a, CKEDITOR.POSITION_AFTER_END)), c.collapse(!0), c.select(), c.scrollIntoView()) : d(a, b, c, e)
				}
			}
		};
	...
	function() {
		...
		CKEDITOR.plugins.add("entities", {
			afterInit: function(f) {
				...
				if (f = (f = f.dataProcessor) && f.htmlFilter) {
					var m = []; ! 1 !== e.basicEntities && m.push("nbsp,gt,lt,amp");
		...
		popup: function(a, f, b, c) {
			...
			if (!l) return ! 1;
			try { - 1 == navigator.userAgent.toLowerCase().indexOf(" chrome/") && (l.moveTo(m, e), l.resizeTo(f, b)),
				l.focus(),
				l.location.href = a
			} catch(d) {
	...
	function() {
		function a(a, b) {
			var c = [];
			if (b) for (var d in b) c.push(d + "\x3d" + encodeURIComponent(b[d]));
			else return a;
		...
		function m(a, d, g, l) {
			if (l && l.length) for (var v, t = l.length; t--;) if (v = l[t], "hbox" != v.type && "vbox" != v.type && "fieldset" != v.type || m(a, d, g, v.children), v.filebrowser) if ("string" == typeof v.filebrowser && (v.filebrowser = {
				action: "fileButton" == v.type ? "QuickUpload": "Browse",
				target: v.filebrowser
			}), "Browse" == v.filebrowser.action) {
				var w = v.filebrowser.url;
				void 0 === w && (w = a.config["filebrowser" + f(d) + "BrowseUrl"], void 0 === w && (w = a.config.filebrowserBrowseUrl));
				w && (v.onClick = b, v.filebrowser.url = w, v.hidden = !1)
			} else if ("QuickUpload" == v.filebrowser.action && v["for"] && (w = v.filebrowser.url, void 0 === w && (w = a.config["filebrowser" + f(d) + "UploadUrl"], void 0 === w && (w = a.config.filebrowserUploadUrl)), w)) {
				var p = v.onClick;
				v.onClick = function(a) {
					var b = a.sender;
					if (p && !1 === p.call(b, a)) return ! 1;
					if (c.call(b, a)) {
				...
				v.filebrowser.url = w;
				v.hidden = !1;
				e(a, g.getContents(v["for"][0]).get(v["for"][1]), v.filebrowser)
			}
		}
		function l(a, b, c) {
			if ( - 1 !== c.indexOf(";")) {
		...
		function d(a, b) {
			...
			if ("function" != typeof b || !1 !== b.call(this._.filebrowserSe)) if (!e || !1 !== e.call(this._.filebrowserSe, a, b)) if ("string" == typeof b && b && alert(b), a && (d = this._.filebrowserSe, c = d.getDialog(), d = d.filebrowser.target || null)) if (d = d.split(":"), e = c.getContentElement(d[0], d[1])) e.setValue(a),
			c.selectPage(d[0])
		}
		...
		function(a) {
			if (a.editor.plugins.filebrowser) for (var b = a.data.definition,
			c, d = 0; d < b.contents.length; ++d) if (c = b.contents[d]) m(a.editor, a.data.name, b, c.elements),
			c.hidden && c.filebrowser && (c.hidden = !l(b, c.id, c.filebrowser))
		})
	} (),
	function() {
		function a(a) {
			...
			l = function() {
				...
				var k, m, p, q, r, z, A, u = e.floatSpaceDockedOffsetX || 0,
				y = e.floatSpaceDockedOffsetY || 0,
				C = e.floatSpacePinnedOffsetX || 0,
				...
				return function(d) {
					if (m = a.editable()) {
						...
						k ? (z + y <= q.top ? g("top") : z + y > r.height - q.bottom ? g("pin") : g("bottom"), d = r.width / 2, d = e.floatSpacePreferRight ? "right": 0 < q.left && q.right < r.width && q.width > p.width ? "rtl" == e.contentsLangDirection ? "right": "left": d - q.left > q.right - d ? "left": "right", p.width > r.width ? (d = "left", n = 0) : (n = "left" == d ? 0 < q.left ? q.left: 0 : q.right < r.width ? r.width - q.right: 0, n + p.width > r.width && (d = "left" == d ? "right": "left", n = 0)), h.setStyle(d, b(("pin" == k ? C: u) + n + ("pin" == k ? 0 : "left" == d ? A: -A)))) : (k = "pin", g("pin"), l(d))
					}
				}
			} ();
			if (m) {
		...
		onLoad: function() {
			...
			CKEDITOR.ui.listBlock = CKEDITOR.tools.createClass({
				...
				$: function(a, b) {
					b = b || {};
					var c = b.attributes || (b.attributes = {}); (this.multiSelect = !!b.multiSelect) && (c["aria-multiselectable"] = !0); ! c.role && (c.role = "listbox");
				...
				proto: {
					...
					showAll: function() {
						...
						for (var f in b) a = c.getById(b[f]),
						d = a.getNext(),
						a.setStyle("display", ""),
	...
	function() {
		...
		CKEDITOR.ui.button.prototype = {
			render: function(a, e) {
				function m() {
					var b = a.mode;
					b && (b = this.modes[b] ? void 0 !== r[b] ? r[b] : CKEDITOR.TRISTATE_OFF: CKEDITOR.TRISTATE_DISABLED, b = a.readOnly && !this.readOnly ? CKEDITOR.TRISTATE_DISABLED: b, this.setState(b), this.refresh && this.refresh())
				}
				var l = CKEDITOR.env,
				...
				w = CKEDITOR.tools.addFunction(function(a) {
					if (t.onkey) return a = new CKEDITOR.dom.event(a),
					!1 !== t.onkey(t, a.getKeystroke())
				}),
				...
				if (this.modes) {
					...
					function() {
						a.mode && this._.state != CKEDITOR.TRISTATE_DISABLED && (r[a.mode] = this._.state)
					},
					this);
					a.on("activeFilterChange", m, this);
					a.on("mode", m, this); ! this.readOnly && a.on("readOnly", m, this)
				} else k && (k = a.getCommand(k)) && (k.on("state",
				function() {
				...
				function(b) {
					...
					e.setAttribute("style", CKEDITOR.skin.getIconStyle(z, "rtl" == b, this.icon, this.iconOffset))
				},
				this);
				k ? (n = a.getCommandKeystroke(k)) && (v = CKEDITOR.tools.keystrokeToString(a.lang.common.keyboard, n)) : h += "off";
				...
				h = {
					...
					title: this.title + (v ? " (" + v.display + ")": ""),
					ariaShortcut: v ? a.lang.common.keyboardShortcut + " " + v.aria: "",
	...
	function() {
		...
		CKEDITOR.ui.richCombo = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				render: function(a, c) {
					...
					a.on("mode", e, this);
					a.on("selectionChange", e, this); ! this.readOnly && a.on("readOnly", e, this);
					var g = CKEDITOR.tools.addFunction(function(c, e) {
						c = new CKEDITOR.dom.event(c);
						var f = c.getKeystroke();
						if (40 == f) a.once("panelShow",
						function(a) {
		...
		init: function(a) {
			if (!a.blockless) {
				for (var f = a.config,
				b = a.lang.format,
				c = f.format_tags.split(";"), e = {},
				m = 0, l = [], d = 0; d < c.length; d++) {
					var h = c[d],
					k = new CKEDITOR.style(f["format_" + h]);
					if (!a.filter.customConfig || a.filter.check(k)) m++,
					e[h] = k,
	...
	function() {
		...
		$: function() {
			...
			var a = CKEDITOR.dtd,
			f;
			for (f in CKEDITOR.tools.extend({},
			a.$nonBodyContent, a.$block, a.$listItem, a.$tableContent)) this.setRules(f, {
				indent: !a[f]["#"],
				breakBeforeOpen: 1,
		...
		CKEDITOR.plugins.add("image", {
			...
			afterInit: function(b) {
				function c(c) {
					...
					if (m) {
						if ("left" == c || "right" == c) m.on("exec",
						function(l) {
							var d = a(b),
							h;
							d && (h = f(d), h == c ? (d.removeStyle("float"), c == f(d) && d.removeAttribute("align")) : d.setStyle("float", c), l.cancel())
						});
						m.on("refresh",
						function(l) {
							var d = a(b);
	...
	function() {
		...
		function b(b, c) {
			...
			for (var h = 0; h < e.length; h++) d = e[h].toLowerCase(),
			f = c[d],
			0 < h && g.push(","),
		...
		CKEDITOR.plugins.link = {
			...
			parseLinkAttributes: function(a, b) {
				...
				y,
				C = {};
				c.match(e) && ("encode" == u ? c = c.replace(n,
				function(a, b, c) {
					c = c || "";
				...
				function(a, b, c) {
					if (b == f.name) {
						...
						c = c.match(/[^,\s]+/g);
						for (var d = c.length,
						e, g, h = 0; h < d; h++) e = decodeURIComponent,
						g = c[h].replace(b, "").replace(/\\'/g, "'"),
						g = e(g),
						e = f.params[h].toLowerCase(),
						a[e] = g;
						a.address = [a.name, a.domain].join("@")
					}
				}));
				if (!C.type) if (u = c.match(h)) C.type = "anchor",
				C.anchor = {},
				C.anchor.name = C.anchor.id = u[1];
				else if (u = c.match(m)) {
					y = c.match(l);
					c = c.match(d);
					C.type = "email";
				...
				} else c && (y = c.match(k)) && (C.type = "url", C.url = {},
				C.url.protocol = y[1], C.url.url = y[2]);
				if (b) {
					if (c = b.getAttribute("target")) C.target = {
						type: c.match(g) ? c: "frame",
						name: c
					};
					else if (c = (c = b.data("cke-pa-onclick") || b.getAttribute("onclick")) && c.match(t)) for (C.target = {
						type: "popup",
						name: c[1]
					}; u = w.exec(c[2]);)"yes" != u[2] && "1" != u[2] || u[1] in {
						height: 1,
						width: 1,
						top: 1,
						left: 1
					} ? isFinite(u[2]) && (C.target[u[1]] = u[2]) : C.target[u[1]] = !0;
					null !== b.getAttribute("download") && (C.download = !0);
					var c = {},
					B;
					for (B in p)(u = b.getAttribute(B)) && (c[p[B]] = u);
			...
			getLinkAttributes: function(c, d) {
				...
				if (d.advanced) {
					for (var t in p)(e = d.advanced[p[t]]) && (g[t] = e);
				...
				d.advanced && CKEDITOR.tools.extend(t, p);
				for (var w in g) delete t[w];
				return {
					set: g,
					removed: CKEDITOR.tools.objectKeys(t)
				}
			},
	...
	function() {
		...
		function d(a) {
			...
			d = CKEDITOR.getUrl(this.path + "images/" + (G.hidpi ? "hidpi/": "") + "icon" + (a.rtl ? "-rtl": "") + ".png");
			B(c, {
				attach: function() {
					this.wrap.getParent() || this.wrap.appendTo(a.editable, !0);
					return this
				},
				lineChildren: [B(F('\x3cspan title\x3d"' + a.editor.lang.magicline.title + '" contenteditable\x3d"false"\x3e\x26#8629;\x3c/span\x3e', b), {
					base: R + "height:17px;width:17px;" + (a.rtl ? "left": "right") + ":17px;background:url(" + d + ") center no-repeat " + a.boxColor + ";cursor:pointer;" + (G.hc ? "font-size: 15px;line-height:14px;border:1px solid #fff;text-align:center;": "") + (G.hidpi ? "background-size: 9px 10px;": ""),
		...
		function h(a, b, c) {
			...
			e = a.editor,
			f;
			G.ie && a.enterMode == CKEDITOR.ENTER_BR ? f = a.doc.createText(da) : (f = (f = l(a.element, !0)) && f.data("cke-enter-mode") || a.enterMode, f = new D(K[f], a.doc), f.is("br") || a.doc.createText(da).appendTo(f));
			...
			return {
				...
				exec: function() {
					...
					return function(f) {
						...
						if (!p(a, f) && f && !f.equals(a.editable) && !f.contains(a.editable)) { (g = l(f)) && "false" == g.getAttribute("contenteditable") && (f = g);
							a.element = f;
							g = e(a, f, !c);
							var h;
		...
		function z(a) {
			...
			else if (m(c.y, h.size.bottom - g, h.size.bottom + 1)) p = !0;
			else return null;
			if (v(h) || q(a, h, p) || h.getParent().is(ca)) return null;
			var r = e(a, h, !p);
			if (r) {
				if (r && r.type == CKEDITOR.NODE_TEXT) return null;
				if (n(r)) {
					if (v(r) || !w(a, r) || r.getParent().is(ca)) return null;
					k = [r, h][p ? "reverse": "concat"]().concat([S, O])
				}
			} else h.equals(a.editable[p ? "getLast": "getFirst"](a.isRelevant)) ? (y(a), p && m(c.y, h.size.bottom - g, d.pane.height) && m(h.size.bottom, d.pane.height - g, d.pane.height) ? l = M: m(c.y, 0, h.size.top + g) && (l = T)) : l = V,
			k = [null, h][p ? "reverse": "concat"]().concat([p ? E: J, O, l, h.equals(a.editable[p ? "getLast": "getFirst"](a.isRelevant)) ? p ? M: T: V]);
			return 0 in k ? new f(k) : null
		}
		function A(a, b, c, d) {
			for (var e = b.getDocumentPosition(), f = {},
			g = {},
			h = {},
			k = {},
			l = ba.length; l--;) f[ba[l]] = parseInt(b.getComputedStyle.call(b, "border-" + ba[l] + "-width"), 10) || 0,
			h[ba[l]] = parseInt(b.getComputedStyle.call(b, "padding-" + ba[l]), 10) || 0,
			g[ba[l]] = parseInt(b.getComputedStyle.call(b, "margin-" + ba[l]), 10) || 0;
			c && !d || C(a, d);
		...
		function x(a, b, c, d) {
			...
			p = a.mouse; p.y + h < m && 0 < p.y - h;) {
				k || (k = b(e, d));
				l || (l = b(g, d)); ! k && 0 < p.y - h && (e = c(a, {
					x: p.x,
					y: p.y - h
				})); ! l && p.y + h < m && (g = c(a, {
					x: p.x,
		...
		CKEDITOR.plugins.add("magicline", {
			init: function(a) {
				...
				function() {
					...
					l.is(H.$inline) || (m.inInlineMode && !t(l) && l.setStyles({
						position: "relative",
						top: null,
						left: null
					}), d.call(this, m), C(m), l.attachListener(a, "beforeUndoImage",
					function() {
						m.line.detach()
					}), l.attachListener(a, "beforeGetData",
					function() {
						m.line.wrap.getParent() && (m.line.detach(), a.once("getData",
						function() {
							m.line.attach()
						},
						null, null, 1E3))
					},
					null, null, 0), l.attachListener(m.inInlineMode ? n: n.getWindow().getFrame(), "mouseout",
					function(b) {
						...
						u = null,
						m.line.detach()
					}), l.attachListener(l, "keyup",
					function() {
						m.hiddenMode = 0
					}), l.attachListener(l, "keydown",
					function(b) {
					...
					}), l.attachListener(m.inInlineMode ? l: n, "mousemove",
					function(b) {
						w = !0;
						if ("wysiwyg" == a.mode && !a.readOnly && !u) {
							...
							u = setTimeout(function() {
								...
								u = m.trigger = null;
								C(m);
								w && !m.hiddenMode && a.focusManager.hasFocus && !m.line.mouseNear() && (m.element = U(m, !0)) && ((m.trigger = r(m) || z(m) || X(m)) && !p(m, m.trigger.upper || m.trigger.lower) ? m.line.attach().place() : (m.trigger = null, m.line.detach()), w = !1)
							},
							30)
						}
					}), l.attachListener(v, "scroll",
					function() {
						"wysiwyg" == a.mode && (m.line.detach(), G.webkit && (m.hiddenMode = 1, clearTimeout(q), q = setTimeout(function() {
							m.mouseDown || (m.hiddenMode = 0)
						},
						50)))
					}), l.attachListener(I ? n: v, "mousedown",
					function() {
						"wysiwyg" == a.mode && (m.line.detach(), m.hiddenMode = 1, m.mouseDown = 1)
					}), l.attachListener(I ? n: v, "mouseup",
					function() {
						m.hiddenMode = 0;
						m.mouseDown = 0
					}), a.addCommand("accessPreviousSpace", k(m)), a.addCommand("accessNextSpace", k(m, !0)), a.setKeystroke([[c.magicline_keystrokePrevious, "accessPreviousSpace"], [c.magicline_keystrokeNext, "accessNextSpace"]]), a.on("loadSnapshot",
					function() {
						var b, c, d, e;
						for (e in {
							p: 1,
		...
		X = function() {
			function b(e) {
				...
				l, p, r, t; h && !h.equals(g) && (p = g.getNext(e.isRelevant));) l = Math.abs(c(e, g, p) - e.mouse.y),
				l < f && (f = l, r = g, t = p),
				g = p,
				u(e, g);
				if (!r || !t || !m(e.mouse.y, r.size.top, t.size.bottom)) return null;
				k.upper = r;
				k.lower = t;
				return k.set(S, 8)
			}
			function d(a, b) {
				return ! (b && b.type == CKEDITOR.NODE_TEXT || aa(b) || v(b) || g(a, b) || b.type == CKEDITOR.NODE_ELEMENT && b.$ && b.is("br"))
			}
			return function(c) {
				var d = b(c),
				e;
				if (e = d) {
					e = d.upper;
					var f = d.lower;
					e = !e || !f || v(f) || v(e) || f.equals(e) || e.equals(f) || f.contains(e) || e.contains(f) ? !1 : w(c, e) && w(c, f) && a(c, e, f) ? !0 : !1
				}
				return e ? d: null
			}
		} (),
			...
			init: function(a) {
				...
				if (a.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
					...
					a.addCommand("maximize", {
						...
						editorFocus: !1,
						exec: function() {
							var w = a.container.getFirst(function(a) {
								return a.type == CKEDITOR.NODE_ELEMENT && a.hasClass("cke_inner")
							}),
							...
							if ("wysiwyg" == a.mode) {
								...
								var r = a.editable().$;
								g = !CKEDITOR.env.ie && [r.selectionStart, r.selectionEnd];
								n = [r.scrollLeft, r.scrollTop]
							}
							if (this.state == CKEDITOR.TRISTATE_OFF) {
								k.on("resize", f);
								v = k.getScrollPosition();
								for (q = a.container; q = q.getParent();) q.setCustomData("maximize_saved_styles", b(q)),
								...
								h.getDocumentElement().setStyles(p); ! CKEDITOR.env.gecko && h.getDocumentElement().setStyle("position", "fixed");
								CKEDITOR.env.gecko && CKEDITOR.env.quirks || h.getBody().setStyles(p);
								...
								for (var q = [p, w], z = 0; z < q.length; z++) c(q[z], q[z].getCustomData("maximize_saved_styles")),
								q[z].removeCustomData("maximize_saved_styles");
								for (q = a.container; q = q.getParent();) c(q, q.getCustomData("maximize_saved_styles")),
								q.removeCustomData("maximize_saved_styles");
								CKEDITOR.env.ie ? setTimeout(function() {
									k.$.scrollTo(v.x, v.y)
								},
								0) : k.$.scrollTo(v.x, v.y);
								w.removeClass("cke_maximized");
								CKEDITOR.env.webkit && (w.setStyle("display", "inline"), setTimeout(function() {
							...
							this.toggleState();
							if (q = this.uiItems[0]) p = this.state == CKEDITOR.TRISTATE_OFF ? d.maximize.maximize: d.maximize.minimize,
							q = CKEDITOR.document.getById(q._.id),
							q.getChild(1).setHtml(p),
							q.setAttribute("title", p),
							q.setAttribute("href", 'javascript:void("' + p + '");');
							"wysiwyg" == a.mode ? g ? (CKEDITOR.env.gecko && e(a), a.getSelection().selectRanges(g), (r = a.getSelection().getStartElement()) && r.scrollIntoView(!0)) : k.$.scrollTo(n.x, n.y) : (g && (r.selectionStart = g[0], r.selectionEnd = g[1]), r.scrollLeft = n[0], r.scrollTop = n[1]);
							g = n = null;
							t = this.state;
			...
			init: function(b) {
				...
				function(f) {
					...
					if (d && (c || h.test(d)) && (!1 !== b.fire("pasteFromWord", k) || c)) {
						...
						function() {
							if (g) b.fire("paste", l);
							else if (!b.config.pasteFromWordPromptCleanup || c || confirm(b.lang.pastefromword.confirmCleanup)) k.dataValue = CKEDITOR.cleanWord(k.dataValue, b),
							b.fire("afterPasteFromWord", k),
							l.dataValue = k.dataValue;
							c = 0
						});
						g && f.cancel()
					}
	...
	function() {
		...
		CKEDITOR.plugins.add("pastetext", {
			...
			init: function(f) {
				...
				if (f.config.forcePasteAsPlainText) f.on("beforePaste",
				function(a) {
				...
				exec: function(a) {
					...
					d; d = m.getNextRange();) {
						...
						if (g) for (n(g), k = k.getNextSourceNode(!0, CKEDITOR.NODE_ELEMENT); k && !k.equals(g);) if (k.isReadOnly()) {
							...
							k = k.getNext(l)
						} else n = k.getNextSourceNode(!1, CKEDITOR.NODE_ELEMENT),
						"img" == k.getName() && k.data("cke-realelement") || !c(a, k) || (f.test(k.getName()) ? k.remove(1) : (k.removeAttributes(b), a.fire("removeFormatCleanup", k))),
						k = n;
						d.moveToBookmark(h)
					}
					a.forceNextSelectionCheck();
	...
	CKEDITOR.config.removeFormatTags = "b,big,cite,code,del,dfn,em,font,i,ins,kbd,q,s,samp,small,span,strike,strong,sub,sup,tt,u,var", CKEDITOR.config.removeFormatAttributes = "class,style,lang,width,height,align,hspace,valign", CKEDITOR.plugins.add("resize", {
		init: function(a) {
			...
			m = a.element ? a.element.getDirection(1) : "ltr"; ! c.resize_dir && (c.resize_dir = "vertical");
			...
			if (!1 !== c.resize_enabled) {
				var l = null,
				d, h, k = ("both" == c.resize_dir || "horizontal" == c.resize_dir) && c.resize_minWidth != c.resize_maxWidth,
				...
				function(b) {
					if ("bottom" == b.data.space) {
						var c = "";
						k && !g && (c = " cke_resizer_horizontal"); ! k && g && (c = " cke_resizer_vertical");
		...
		addMenuItems: function(a) {
			...
			a.plugins.wsc && (c.WSC = {
				...
				onClick: function() {
					...
					c = a.scayt,
					d = a.elementMode == CKEDITOR.ELEMENT_MODE_INLINE ? a.container.getText() : a.document.getBody().getText(); (d = d.replace(/\s/g, "")) ? (c && b.state.scayt[a.name] && c.setMarkupPaused && c.setMarkupPaused(!0), a.lockSelection(), a.execCommand("checkspell")) : alert("Nothing to check!")
				}
			});
			a.addMenuItems(c)
		},
		bindEvents: function(a) {
			...
			e = function() { ! f.state.scayt[a.name] || a.readOnly || a.scayt || f.createScayt(a)
			},
			m = function() {
				var c = a.editable();
				c.attachListener(c, "focus",
				function(c) {
					...
					if ((b || c) && a._.savedSelection) {
						...
						for (var l = 0; l < c.length; l++) f = c[l],
						"string" === typeof f.startContainer.$.nodeValue && (d = f.startContainer.getText().length, (d < f.startOffset || d < f.endOffset) && a.unlockSelection(!1))
					}
				},
				this, null, -10)
			},
			l = function() {
				b ? a.config.scayt_inlineModeImmediateMarkup ? e() : (a.on("blur",
				function() {
					setTimeout(c, 0)
				}), a.on("focus", e), a.focusManager.hasFocus && e()) : e();
				m();
				var d = a.editable();
				d.attachListener(d, "mousedown",
				function(b) {
					b = b.data.getTarget();
			...
			a.on("setData",
			function() {
				c(); (a.elementMode == CKEDITOR.ELEMENT_MODE_INLINE || a.plugins.divarea) && l()
			},
			this, null, 50);
			a.on("reloadMarkupScayt",
			function(b) {
		...
		parseConfig: function(a) {
			...
			if ("string" === typeof a.config.scayt_uiTabs && 3 === a.config.scayt_uiTabs.split(",").length) {
				...
				CKEDITOR.tools.search(a.config.scayt_uiTabs,
				function(a) {
		...
		buildSuggestionMenuItems: function(a, f, b) {
			...
			d = a.scayt;
			if (0 < f.length && "no_any_suggestions" !== f[0]) if (b) for (b = 0; b < f.length; b++) {
			...
			} else for (b = 0; b < f.length; b++) h = "grayt_suggest_" + CKEDITOR.plugins.scayt.suggestions[b].replace(" ", "_"),
			a.addCommand(h, this.createCommand(CKEDITOR.plugins.scayt.suggestions[b], m, l)),
			a.addMenuItem(h, {
				label: f[b],
				command: h,
			...
			else c.no_scayt_suggest = CKEDITOR.TRISTATE_DISABLED,
		...
		menuGenerator: function(a, f) {
			...
			l = f.getAttribute(b.getLangAttribute()) || b.getLang(),
			d,
			h;
			d = b.isScaytNode(f);
			h = b.isGraytNode(f);
			...
			function(a) {
				...
				if (m) {
					...
					try {
						...
						f.alarmCompatibilityMessage(),
						delete m.charsToObserve,
						l = e(m)
					}
					l.subscribe("suggestionListSend",
					function(a) {
						for (var b = {},
						c = [], d = 0; d < a.suggestionList.length; d++) b["word_" + a.suggestionList[d]] || (b["word_" + a.suggestionList[d]] = a.suggestionList[d], c.push(a.suggestionList[d]));
						CKEDITOR.plugins.scayt.suggestions = c
					});
					l.subscribe("selectionIsChanged",
					function(b) {
						a.getSelection().isLocked && a.lockSelection()
					});
					l.subscribe("graytStateChanged",
					function(d) {
		...
		loadScaytLibrary: function(a, f) {
			...
			"undefined" === typeof window.SCAYT || "function" !== typeof window.SCAYT.CKSCAYT ? (b = a.config.scayt_srcUrl + "?" + this.onLoadTimestamp, CKEDITOR.scriptLoader.load(b,
			function(a) {
				a && c()
	...
	function() {
		if (!0 === CKEDITOR.config.scayt_handleCheckDirty) {
			...
			a.checkDirty = CKEDITOR.tools.override(a.checkDirty,
			function(a) {
				return function() {
					var c = null,
					e = this.scayt;
		...
		if (!0 === CKEDITOR.config.scayt_handleUndoRedo) {
			...
			a[f] = CKEDITOR.tools.override(a[f],
			function(a) {
				return function(c) {
	...
	function() {
		...
		CKEDITOR.plugins.add("showborders", {
			...
			init: function(f) {
				...
				b.canUndo = !1; ! 1 !== f.config.startupShowBorders && b.setState(CKEDITOR.TRISTATE_ON);
				f.on("mode",
				function() {
		...
		CKEDITOR.plugins.add("stylescombo", {
			...
			init: function(a) {
				...
				function(b) {
					if (b = b.data.styles) {
						for (var d, h, k, g = 0,
						n = b.length; g < n; g++)(d = b[g], a.blockless && d.element in CKEDITOR.dtd.$block || "string" == typeof d.type && !CKEDITOR.style.customHandlers[d.type] || (h = d.name, d = new CKEDITOR.style(d), a.filter.customConfig && !a.filter.check(d))) || (d._name = h, d._.enterMode = f.enterMode, d._.type = k = d.assignedTo || d.type, d._.weight = g + 1E3 * (k == CKEDITOR.STYLE_OBJECT ? 1 : k == CKEDITOR.STYLE_BLOCK ? 2 : 3), c[h] = d, e.push(d), m.push(d));
						e.sort(function(a, b) {
				...
				a.ui.addRichCombo("Styles", {
					...
					init: function() {
						var a, c, f, k, g, m;
						g = 0;
						for (m = e.length; g < m; g++) a = e[g],
						c = a._name,
						k = a._.type,
						k != f && (this.startGroup(b["panelTitle" + String(k)]), f = k),
	...
	function() {
				...
				exec: function(b) {
					if (b.editable().hasFocus) {
						...
						1)) {
							...
							if (f || a) if (f) f = new CKEDITOR.dom.element(f),
							c.moveToElementEditStart(f),
							c.checkStartOfBlock() && c.checkEndOfBlock() || c.selectNodeContents(f);
							else return ! 0;
							else {
								for (var k = d.getAscendant("table").$, f = d.getParent().$.cells, k = new CKEDITOR.dom.element(k.insertRow( - 1), b.document), g = 0, n = f.length; g < n; g++) k.append((new CKEDITOR.dom.element(f[g], b.document)).clone(!1, !1)).appendBogus();
		...
		CKEDITOR.plugins.add("tab", {
			init: function(e) {
				for (var m = !1 !== e.config.enableTabKeyTools,
				l = e.config.tabSpaces || 0,
				d = ""; l--;) d += " ";
				if (d) e.on("key",
				function(a) {
				...
				a.addCommand("tableDelete", f({
					exec: function(a) {
						...
						if (b) {
							var f = b.getParent(),
							l = a.editable();
							1 != f.getChildCount() || f.is("td", "th") || f.equals(l) || (b = f);
							a = a.createRange();
	...
	function() {
			...
			if (c instanceof CKEDITOR.dom.selection) {
				...
				for (var d = d[d.length - 1], g = d.getParent().$.rowIndex + d.$.rowSpan - 1, d = [], h = c; h <= g; h++) {
					for (var k = f[h], l = new CKEDITOR.dom.element(e.$.rows[h]), m = 0; m < k.length; m++) {
						...
						v = n.getParent().$.rowIndex;
						1 == n.$.rowSpan ? n.remove() : (--n.$.rowSpan, v == h && (v = f[h + 1], v[m - 1] ? n.insertAfter(new CKEDITOR.dom.element(v[m - 1])) : (new CKEDITOR.dom.element(e.$.rows[h + 1])).append(n, 1)));
						m += n.$.colSpan - 1
					}
					d.push(l)
				}
		...
		function l(b) {
			if (b instanceof CKEDITOR.dom.selection) {
				...
				a: {
					...
					e = b.length - 1;
					for (var g = {},
					h, k; h = b[f++];) CKEDITOR.dom.element.setMarker(g, h, "delete_cell", !0);
					for (f = 0; h = b[f++];) if ((k = h.getPrevious()) && !k.getCustomData("delete_cell") || (k = h.getNext()) && !k.getCustomData("delete_cell")) {
						CKEDITOR.dom.element.clearAllMarkers(g);
						e = k;
						break a
					}
					CKEDITOR.dom.element.clearAllMarkers(g);
					k = b[0].getParent(); (k = k.getPrevious()) ? e = k.getLast() : (k = b[e].getParent(), e = (k = k.getNext()) ? k.getChild(0) : null)
				}
		...
		function k(b, c, d) {
			...
			if (c) {
				...
				try {
					...
					B = k["up" == c ? n - D: "down" == c ? n + D: n]["left" == c ? v - g: "right" == c ? v + g: v]
				} catch(F) {
					return ! 1
				}
				if (!B || b.$ == B) return ! 1;
				e["up" == c || "left" == c ? "unshift": "push"](new CKEDITOR.dom.element(B))
			}
			c = b.getDocument();
			var G = n,
			D = B = 0,
			...
			for (c = 0; c < e.length; c++) {
				...
				D = Math.max(D, O - v + E);
				B = Math.max(B, K - n + S);
				d || (E = g, (S = E.getBogus()) && S.remove(), E.trim(), g.getChildren().count() && (K == G || !J || J.isBlockBoundary && J.isBlockBoundary({
					br: 1
			...
			I.moveChildren(b);
			b.appendBogus();
			D >= m ? b.removeAttribute("rowSpan") : b.$.rowSpan = B;
			B >= l ? b.removeAttribute("colSpan") : b.$.colSpan = D;
		...
		function g(b, c) {
			...
			if (1 < m) {
				n = Math.ceil(m / 2);
				for (var m = Math.floor(m / 2), e = k + n, f = new CKEDITOR.dom.element(f.$.rows[e]), g = h(g, e), v, e = d.clone(), k = 0; k < g.length; k++) if (v = g[k], v.parentNode == f.$ && k > l) {
					e.insertBefore(new CKEDITOR.dom.element(v));
					break
				} else v = null;
				v || f.append(e)
			} else for (m = n = 1, f = e.clone(), f.insertAfter(e), f.append(e = d.clone()), v = h(g, k), l = 0; l < v.length; l++) v[l].rowSpan++;
			e.appendBogus();
			d.$.rowSpan = n;
			e.$.rowSpan = m;
			...
			init: function(c) {
				...
				p("columnDelete", h({
					...
					exec: function(b) {
						...
						for (var f = CKEDITOR.tools.buildTableMap(b), g, h, k = [], l = 0, m = f.length; l < m; l++) for (var n = 0,
						p = f[l].length; n < p; n++) f[l][n] == c.$ && (g = n),
						f[l][n] == e.$ && (h = n);
						for (l = g; l <= h; l++) for (n = 0; n < f.length; n++) e = f[n],
						c = new CKEDITOR.dom.element(b.$.rows[n]),
						e = new CKEDITOR.dom.element(e[l]),
						e.$ && (1 == e.$.colSpan ? e.remove() : --e.$.colSpan, n += e.$.rowSpan - 1, c.$.cells.length || k.push(c));
				...
				c.addMenuItems && c.addMenuItems({
					...
					tablecell_insertAfter: {
						label: q.cell.insertAfter,
						group: "tablecell",
						command: "cellInsertAfter",
						order: 10
					},
					tablecell_delete: {
						label: q.cell.deleteCell,
						group: "tablecell",
						command: "cellDelete",
						order: 15
					},
					...
					tablecell_merge_right: {
						label: q.cell.mergeRight,
						group: "tablecell",
						command: "cellMergeRight",
						order: 17
					},
					tablecell_merge_down: {
						label: q.cell.mergeDown,
						group: "tablecell",
						command: "cellMergeDown",
						order: 18
					},
					tablecell_split_horizontal: {
						label: q.cell.splitHorizontal,
						group: "tablecell",
						command: "cellHorizontalSplit",
						order: 19
					},
					tablecell_split_vertical: {
						label: q.cell.splitVertical,
						group: "tablecell",
					...
					tablerow_insertBefore: {
						label: q.row.insertBefore,
						group: "tablerow",
						command: "rowInsertBefore",
						order: 5
					},
					tablerow_insertAfter: {
						label: q.row.insertAfter,
						group: "tablerow",
		...
		function a(a) {
			function b() {
				for (var g = c(), h = CKEDITOR.tools.clone(a.config.toolbarGroups) || f(a), k = 0; k < h.length; k++) {
					...
					if ("/" != m) {
						...
						var p, q = m.groups;
						if (q) for (var r = 0; r < q.length; r++) p = q[r],
						(p = g[p]) && d(m, p); (p = g[m.name]) && d(m, p)
					}
				}
				return h
			}
			...
			function d(b, c) {
				if (c.length) {
					...
					k && -1 != CKEDITOR.tools.indexOf(k, d) || (d = a.ui.create(d)) && a.addFeature(d) && b.items.push(d)
				}
			}
			function h(a) {
				var b = [],
				...
				f = {},
				"/" == e ? b.push(e) : CKEDITOR.tools.isArray(e) ? (d(f, CKEDITOR.tools.clone(e)), b.push(f)) : e.items && (d(f, CKEDITOR.tools.clone(e.items)), f.name = e.name, b.push(f));
				return b
			}
		...
		CKEDITOR.plugins.add("toolbar", {
			...
			init: function(e) {
				var f, l = function(a, b) {
					...
					switch (b) {
					case 9:
					case CKEDITOR.SHIFT + 9 : for (; ! c || !c.items.length;) if (c = 9 == b ? (c ? c.next: a.toolbar.next) || e.toolbox.toolbars[0] : (c ? c.previous: a.toolbar.previous) || e.toolbox.toolbars[e.toolbox.toolbars.length - 1], c.items.length) for (a = c.items[f ? c.items.length - 1 : 0]; a && !a.focus;)(a = f ? a.previous: a.next) || (c = 0);
						a && a.focus();
						return ! 1;
					case v:
						c = a;
				...
				function(c) {
					if (c.data.space == e.config.toolbarLocation) {
						...
						k = ['\x3cspan id\x3d"', f, '" class\x3d"cke_voice_label"\x3e', e.lang.toolbar.toolbars, "\x3c/span\x3e", '\x3cspan id\x3d"' + e.ui.spaceId("toolbox") + '" class\x3d"cke_toolbox" role\x3d"group" aria-labelledby\x3d"', f, '" onmousedown\x3d"return false;"\x3e'],
						...
						for (var v = e.toolbox.toolbars,
						t = a(e), w = t.length, p = 0; p < w; p++) {
							var q, r = 0,
							z, A = t[p],
							u = "/" !== A && ("/" === t[p + 1] || p == w - 1),
							y;
							if (A) if (g && (k.push("\x3c/span\x3e"), m = g = 0), "/" === A) k.push('\x3cspan class\x3d"cke_toolbar_break"\x3e\x3c/span\x3e');
							else {
								y = A.items || A;
								for (var C = 0; C < y.length; C++) {
									...
									if (x) {
										var D = function(a) {
											a = a.render(e, k);
											F = r.items.push(a) - 1;
											0 < F && (a.previous = r.items[F - 1], a.previous.next = a);
											a.toolbar = r;
											a.onkey = l;
											a.onfocus = function() {
										...
										if (x.type == CKEDITOR.UI_SEPARATOR) m = g && x;
											...
											if (!r) {
												...
												z = A.name && (e.lang.toolbar.toolbarGroups[A.name] || A.name);
												k.push('\x3cspan id\x3d"', q, '" class\x3d"cke_toolbar' + (u ? ' cke_toolbar_last"': '"'), z ? ' aria-labelledby\x3d"' + q + '_label"': "", ' role\x3d"toolbar"\x3e');
												z && k.push('\x3cspan id\x3d"', q, '_label" class\x3d"cke_voice_label"\x3e', z, "\x3c/span\x3e");
												k.push('\x3cspan class\x3d"cke_toolbar_start"\x3e\x3c/span\x3e');
												var F = v.push(r) - 1;
												0 < F && (r.previous = v[F - 1], r.previous.next = r)
											}
											B ? g || (k.push('\x3cspan class\x3d"cke_toolgroup" role\x3d"presentation"\x3e'), g = 1) : g && (k.push("\x3c/span\x3e"), g = 0);
											m && (D(m), m = 0);
											D(x)
										}
									}
						...
						if (e.config.toolbarCanCollapse && e.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
							...
							k.push('" onclick\x3d"CKEDITOR.tools.callFunction(' + G + ')"\x3e', '\x3cspan class\x3d"cke_arrow"\x3e\x26#9650;\x3c/span\x3e', "\x3c/a\x3e")
						}
						k.push("\x3c/span\x3e");
						c.data.html += k.join("")
					}
				...
				function() {
					if (this.toolbox) {
						var a, b = 0,
						c, e, f;
						for (a = this.toolbox.toolbars; b < a.length; b++) for (e = a[b].items, c = 0; c < e.length; c++) f = e[c],
	...
	function() {
		...
		CKEDITOR.plugins.add("undo", {
			init: function(c) {
				...
				g.onChange = function() {
					v.setState(g.undoable() ? CKEDITOR.TRISTATE_OFF: CKEDITOR.TRISTATE_DISABLED);
		...
		b.prototype = {
			...
			save: function(a, b, e) {
				...
				this.currentImage = b; ! 1 !== e && this.refreshState();
				return ! 0
			},
			...
			getNextImage: function(a) {
				...
				if (c) if (a) for (e = this.index - 1; 0 <= e; e--) {
					if (a = b[e], !c.equalsContent(a)) return a.index = e,
					a
				} else for (e = this.index + 1; e < b.length; e++) if (a = b[e], !c.equalsContent(a)) return a.index = e,
				a;
				return null
			},
			redoable: function() {
			...
			update: function(a) {
				if (!this.locked) {
					...
					e = this.snapshots; 0 < b && this.currentImage.equalsContent(e[b - 1]);)--b;
		...
		m.prototype = {
			onKeydown: function(d) {
				var e = d.data.getKey();
				if (229 !== e) if ( - 1 < CKEDITOR.tools.indexOf(a, d.data.getKeystroke())) d.data.preventDefault();
				else if (this.keyEventsStack.cleanUp(d), d = this.undoManager, this.keyEventsStack.getLast(e) || this.keyEventsStack.push(e), this.lastKeydownImage = new c(d.editor), b.isNavigationKey(e) || this.undoManager.keyGroupChanged(e)) if (d.strokesRecorded[0] || d.strokesRecorded[1]) d.save(!1, this.lastKeydownImage, !1),
				d.resetType()
			},
			onInput: function() {
			...
			onKeyup: function(a) {
				...
				if (! (b.ieFunctionalKeysBug(a) && this.lastKeydownImage && this.lastKeydownImage.equalsContent(new c(e.editor, !0)))) if (0 < f) e.type(a);
				else if (b.isNavigationKey(a)) this.onNavigationKey(!0)
			},
			onNavigationKey: function(a) {
				var b = this.undoManager; ! a && b.save(!0, null, !1) || b.updateSelection(new c(b.editor));
				b.resetType()
			},
			...
			attachListeners: function() {
				...
				c.attachListener(c.isInline() ? c: a.document.getDocumentElement(), "click",
				function() {
					e.onNavigationKey()
				},
				null, null, 999);
				c.attachListener(this.undoManager.editor, "blur",
				function() {
		...
		l.prototype = {
			...
			getTotalInputs: function() {
				for (var a = this.stack.length,
				b = 0; a--;) b += this.stack[a].inputs;
				return b
			},
			cleanUp: function(a) {
				a = a.data.$;
	...
	function() {
		function a(a) {
			...
			k && k.parentNode.removeChild(k); (k = d.getElementById("cke_shimscrpt")) && k.parentNode.removeChild(k); (k = d.getElementById("cke_basetagscrpt")) && k.parentNode.removeChild(k);
			h.contentEditable = !0;
			...
			if (CKEDITOR.env.webkit || CKEDITOR.env.ie && 10 < CKEDITOR.env.version) d.getDocumentElement().on("mousedown",
			function(a) {
			...
			CKEDITOR.env.iOS && this.attachListener(d, "touchend",
			function() {
				a.focus()
			});
			...
			proto: {
				setData: function(a, c) {
					...
					f.fire("dataReady");
						...
						h ? (/<body[\s|>]/.test(a) || (a = "\x3cbody\x3e" + a), /<html[\s|>]/.test(a) || (a = "\x3chtml\x3e" + a + "\x3c/html\x3e"), /<head[\s|>]/.test(a) ? /<title[\s|>]/.test(a) || (a = a.replace(/<head[^>]*>/, "$\x26\x3ctitle\x3e\x3c/title\x3e")) : a = a.replace(/<html[^>]*>/, "$\x26\x3chead\x3e\x3ctitle\x3e\x3c/title\x3e\x3c/head\x3e"), n && (a = a.replace(/<head[^>]*?>/, "$\x26" + n)), a = a.replace(/<\/head\s*>/, g + "$\x26"), a = k + a) : a = d.docType + '\x3chtml dir\x3d"' + d.contentsLangDirection + '" lang\x3d"' + (d.contentsLanguage || f.langCode) + '"\x3e\x3chead\x3e\x3ctitle\x3e' + this._.docTitle + "\x3c/title\x3e" + n + g + "\x3c/head\x3e\x3cbody" + (d.bodyId ? ' id\x3d"' + d.bodyId + '"': "") + (d.bodyClass ? ' class\x3d"' + d.bodyClass + '"': "") + "\x3e" + a + "\x3c/body\x3e\x3c/html\x3e";
